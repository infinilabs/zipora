...
impl Bmi2SelectOps {
    ...
    fn select1_binary_search_optimized(word: u64, k: u32) -> Option<u32> {
        ...
        if low > 0 && (word >> (low - 1)) & 1 == 1 {
            let rank_before = if low == 1 {
                0
            } else {
                Self::popcount_range(word, 0, low - 1)
            };
            if rank_before == k {
                return Some(low - 1);
            }
    ...
    fn popcount_range(word: u64, start: u32, end: u32) -> u32 {
        ...
        let start_mask = if start == 0 { 0 } else { (1u64 << start) - 1 };
    ...
    pub fn select1_hybrid_cache(
        ...
        let start_block = if cache_idx == 0 {
            0
        } else {
        ...
        if search_range <= 32 {
            ...
            for block_idx in start_block..end_block.min(rank_cache.len() as u32) {
                if rank_cache[block_idx as usize] > target_rank {
                    return Self::find_select_in_block(
                        bit_data,
                        block_idx as usize,
                        target_rank,
                        rank_cache,
    ...
    fn find_select_in_block(
        ...
        Err(ZiporaError::invalid_data(
            "Select position not found in block",
        ))
    }
...
impl Bmi2AdvancedPatterns {
    ...
    pub fn pdep_ctz_select_bulk(word: u64, indices: &[u32]) -> Result<Vec<u32>> {
        ...
        for &k in indices {
            if k >= word_popcount {
                return Err(ZiporaError::invalid_data(format!(
                    "Select index {} out of bounds (word has {} ones)",
                    k, word_popcount
                )));
            }
...
impl Bmi2Dispatcher {
    ...
    fn get_available_operations(&self) -> Vec<&'static str> {
        ...
        if self.capabilities.has_bmi2 {
            ops.extend_from_slice(&["PDEP/PEXT", "BZHI", "PDEP+CTZ select", "Advanced patterns"]);
        }
...
impl Bmi2Accelerator {
    ...
    pub fn extract_cache_aligned_field(
        &self,
        cache_line: u64,
        field_offset: u32,
        field_size: u32,
...
impl Bmi2OperationTrait for PopcountOperation {
    ...
    fn estimated_speedup(&self) -> f64 {
        if self.uses_bmi2_acceleration() {
            2.0
        } else {
            1.0
        }
...
impl Bmi2OperationTrait for SelectOperation {
    fn execute_bmi2(&self, input: &[u64]) -> Vec<u64> {
        ...
        input
            .iter()
            .map(|&word| Bmi2AdvancedPatterns::pdep_ctz_select(word, 0).unwrap_or(u32::MAX) as u64)
            .collect()
    }
...
mod tests {
    ...
    fn test_pext_parallel_extract() {
        let sources = vec![0b11110000u64, 0b10101010u64, 0b11001100u64, 0b11111111u64];
    ...
    fn test_edge_cases() {
        ...
        assert_eq!(
            Bmi2AdvancedPatterns::pdep_ctz_select(all_ones, 63),
            Some(63)
        );
        ...
        assert_eq!(Bmi2BzhiOps::popcount_bzhi_enhanced(word, 0), 0);
        assert_eq!(
            Bmi2BzhiOps::popcount_bzhi_enhanced(word, 100),
            word.count_ones()
        ); // Clamp
    }
    ...
    fn test_performance_comparison() {
        ...
        let test_words = (0..1000)
            .map(|i| (i as u64).wrapping_mul(0x123456789ABCDEFu64))
            .collect::<Vec<_>>();

        // Test different operation types
        let start = std::time::Instant::now();
        let _popcount_results: Vec<u32> = test_words
            .iter()
            .map(|&w| Bmi2RankOps::popcount_u64(w))
            .collect();
        ...
        let start = std::time::Instant::now();
        let _select_results: Vec<Option<u32>> = test_words
            .iter()
            .filter(|&&w| w != 0)
        ...
        let start = std::time::Instant::now();
        let _extract_results: Vec<u64> = test_words
            .iter()
