...
mod tests {
    ...
    fn test_varint_encoded_len() {
        ...
        for (value, expected_len) in test_cases {
            ...
            let encoded = VarInt::encode(value);
            assert_eq!(encoded.len(), expected_len, "Actual encoding length differs for {}", value);
        }
    }
    ...
    fn test_signed_varint_zigzag() {
        ...
        for (signed_val, expected_unsigned) in test_cases {
            ...
            assert_eq!(encoded, unsigned_encoded, "Zigzag encoding failed for {}", signed_val);
        }
    }
    ...
    fn test_varint_max_encoded_len() {
        ...
        for &value in &test_values {
            let encoded = VarInt::encode(value);
            assert!(encoded.len() <= VarInt::MAX_ENCODED_LEN, 
                   "Value {} encoded to {} bytes, exceeds max {}", 
                   value, encoded.len(), VarInt::MAX_ENCODED_LEN);
        }
    }
}