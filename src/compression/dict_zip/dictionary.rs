...
impl SuffixArrayDictionary {
    ...
    pub fn new(training_data: &[u8], config: SuffixArrayDictionaryConfig) -> Result<Self> {
        ...
        let suffix_array = Arc::new(SuffixArray::with_config(
            &sampled_data,
            &config.suffix_array_config,
        )?);
    ...
    pub fn find_longest_match(
        ...
        let result =
            if match_status.depth >= self.config.min_pattern_length && !match_status.is_empty() {
                ...
                self.stats
                    .update_search(false, 0, start_time.elapsed().as_micros() as u64);
                None
            };
    ...
    pub fn save_to_file<P: AsRef<std::path::Path>>(&self, path: P) -> Result<()> {
        ...
        let mut file = File::create(path).map_err(|e| {
            ZiporaError::io_error(format!("Failed to create dictionary file: {}", e))
        })?;

        file.write_all(&serialized).map_err(|e| {
            ZiporaError::io_error(format!("Failed to write dictionary file: {}", e))
        })?;
    ...
    fn sa_equal_range_binary_optimized(
        &self,
        lo: usize,
        hi: usize,
        pos: usize,
...
mod tests {
    ...
    fn test_debug_pattern_matching() {
        ...
        println!(
            "Dictionary text length: {}",
            dictionary.dictionary_text.len()
        );
        println!(
            "Suffix array length: {}",
            dictionary.suffix_array.as_slice().len()
        );
        ...
        for i in 0..std::cmp::min(10, dictionary.suffix_array.as_slice().len()) {
            ...
            if suffix_idx < dictionary.dictionary_text.len() {
                ...
                let suffix_str =
                    String::from_utf8_lossy(&suffix[..std::cmp::min(10, suffix.len())]);
    ...
    fn test_comprehensive_two_level_algorithm() {
        ...
        for (pattern, should_match) in &test_cases {
            ...
            println!(
                "  da_match_max_length: lo={}, hi={}, depth={}",
            ...
            let result = dictionary
                .find_longest_match(pattern, 0, pattern.len())
                .unwrap();
            if *should_match {
                if result.is_none() {
    ...
    fn test_focused_binary_search_bug() {
        ...
        println!(
            "\nSearching for '{}' at position {} in range [{}, {})",
            ch as char, pos, lo, hi
        );
        ...
        if linear_result != binary_result {
            ...
            while lo_search < hi_search {
                let mid = lo_search + (hi_search - lo_search) / 2;
                println!(
                    "  Checking mid={} in range [{}, {})",
                    mid, lo_search, hi_search
                );
    ...
    fn test_debug_binary_search_issue() {
        ...
        while pos < pattern.len() {
            ...
            if let Some(next_state) = child {
                if let Some(dfa_state) = dictionary.dfa_cache.get_state(next_state) {
                    println!(
                        "  DFA cache HIT: next_state={}, new range=({}, {})",
        ...
        if pos < pattern.len() {
            println!(
                "\nContinuing with suffix array from pos={}, range=({}, {})",
                pos, lo, hi
            );
            ...
            while pos < pattern.len() && lo < hi {
                let ch = pattern[pos];
                println!(
                    "  SA Step {}: Looking for '{}' at position {}",
                    pos, ch as char, pos
                );
        ...
        if actual_result.depth != pos {
            println!(
                "WARNING: Manual trace depth ({}) differs from actual result ({})",
                ...
                if let Some(&suffix_idx) = dictionary.suffix_array.as_slice().get(i) {
                    if suffix_idx < dictionary.dictionary_text.len() {
                        ...
                        let text_preview = if suffix_idx < dictionary.dictionary_text.len() {
                            ...
                            std::str::from_utf8(&dictionary.dictionary_text[suffix_idx..end])
                                .unwrap_or("<invalid>")
                        } else {
                            "<out of bounds>"
                        };
        ...
        if match_status.is_empty() {
            ...
            for (i, &ch) in pattern.iter().enumerate() {
                println!(
                    "  Step {}: searching for '{}' at position {}",
                    i, ch as char, i
                );
        ...
        let dict1 = SuffixArrayDictionary::new(
            training_data,
        ...
        let binary_only =
            dict1.sa_equal_range_binary_optimized(0, dict1.suffix_array.as_slice().len(), 0, b't');
        println!("Binary only result: ({}, {})", binary_only.0, binary_only.1);

        // Fresh dictionary - linear then binary
        let dict2 = SuffixArrayDictionary::new(
            training_data,
            SuffixArrayDictionaryConfig {
                min_pattern_length: 3,
                max_pattern_length: 25,
        ...
        let binary_after_linear =
            dict2.sa_equal_range_binary_optimized(0, dict2.suffix_array.as_slice().len(), 0, b't');
        println!(
            "Binary after linear result: ({}, {})",
            binary_after_linear.0, binary_after_linear.1
        );
