...
use crate::compression::dict_zip::{
    compression_types::{
        CompressionType, Match, calculate_encoding_cost, choose_best_compression_type,
    },
    dfa_cache::CacheStats,
    dictionary::{MatchStats, SuffixArrayDictionary},
    local_matcher::{LocalMatch, LocalMatcher, LocalMatcherConfig, LocalMatcherStats},
    ...
    pub fn finalize(&mut self, start_time: Instant) {
        ...
        if self.bytes_processed > 0 {
            ...
            self.processing_speed =
                self.bytes_processed as f64 / self.compression_time.as_secs_f64();
        }
...
impl PaZipCompressor {
    ...
    fn compress_sequential_legacy(&mut self, input: &[u8], output: &mut Vec<u8>) -> Result<()> {
        ...
        while pos < input.len() {
            ...
            let strategies =
                self.calculate_strategy_costs(input, pos, local_match, global_match)?;
    ...
    fn find_local_match(&mut self, input: &[u8], pos: usize) -> Result<Option<LocalMatch>> {
        ...
        self.local_matcher.find_match(
            remaining,
            self.config.max_local_probe_distance as usize,
            max_length,
        )
    }

    /// Step 2: Find global match using dictionary
    fn find_global_match(
        &mut self,
        input: &[u8],
        pos: usize,
    ...
    fn calculate_local_match_cost(
        &self,
        local_match: LocalMatch,
    ) -> Result<Option<(CompressionStrategy, CostAnalysis)>> {
        // Determine compression type based on distance and length
        let compression_type =
            match choose_best_compression_type(local_match.distance, local_match.length) {
                Some(ct) => ct,
        ...
        let cost_analysis = CostAnalysis {
            ...
            efficiency: if local_match.length > 0 {
                (local_match.length as f64 * 8.0 - total_cost as f64)
                    / (local_match.length as f64 * 8.0)
            } else {
                0.0
            },
        };
    ...
    fn calculate_global_match_cost(
        &self,
        global_match: crate::compression::dict_zip::matcher::Match,
    ) -> Result<Option<(CompressionStrategy, CostAnalysis)>> {
        ...
        let cost_analysis = CostAnalysis {
            ...
            efficiency: if global_match.length > 0 {
                (global_match.length as f64 * 8.0 - total_cost as f64)
                    / (global_match.length as f64 * 8.0)
            } else {
                0.0
            },
        };
    ...
    fn select_optimal_strategy(
        ...
        for (strategy, analysis) in strategies {
            ...
            if adjusted_benefit >= self.adaptive_thresholds.min_net_benefit
                && adjusted_benefit > best_benefit
            {
    ...
    fn update_adaptive_thresholds(&mut self, strategy: CompressionStrategy) {
        ...
        self.adaptive_thresholds
            .update(efficiency, strategy, self.config.learning_rate);
...
mod tests {
    ...
    fn test_cost_analysis() -> Result<()> {
        ...
        assert_eq!(
            literal_cost.encoding_cost,
            compressor.config.literal_cost_bits * 5
        );
    ...
    fn test_strategy_selection() -> Result<()> {
        ...
        let strategies = vec![
            (
                CompressionStrategy::Literal { length: 1 },
                CostAnalysis {
                    net_benefit: -8,
                    encoding_cost: 8,
                    access_cost: 0,
...
mod reference_compliance_tests {
    ...
    fn test_reference_compliant_compression_basic() -> Result<()> {
        ...
        assert!(
            ...
            "Should not process more than input"
        );
        ...
        assert!(
            output.len() >= 4,
            "Output should have at least some encoding overhead"
        );

        // Print compression details for validation
        println!(
            "Reference compression: {} -> {} bytes (ratio: {:.3})",
            input.len(),
            output.len(),
            stats.compression_ratio
        );
        println!(
            "Output format validation: first 10 bytes = {:?}",
    ...
    fn test_reference_compliant_compression_patterns() -> Result<()> {
        ...
        for (i, input) in test_cases.iter().enumerate() {
            ...
            assert!(
                output.len() >= 2,
                "Test case {} should have minimum encoding overhead",
                i
            );

            println!(
                "Test case {}: input len={}, output len={}, ratio={:.3}",
                i,
                input.len(),
                output.len(),
                stats.compression_ratio
    ...
    fn test_reference_compliant_compression_type_usage() -> Result<()> {
        ...
        println!(
            "Compression stats: global={}, local={}, literals={}",
    ...
    fn test_reference_vs_legacy_compression() -> Result<()> {
        ...
        assert!(
            ref_output.len() >= 4,
            "Reference compression should have minimum overhead"
        );
        assert!(
            legacy_output.len() >= 4,
            "Legacy compression should have minimum overhead"
        );
        ...
        println!(
            ...
            input.len(),
            ref_output.len(),
            ref_stats.compression_ratio
        );
        println!(
            "Legacy compression: {} -> {} bytes (ratio: {:.3})",
            input.len(),
            legacy_output.len(),
            legacy_stats.compression_ratio
        );
    ...
    fn test_reference_compliant_edge_cases() -> Result<()> {
        ...
        for (i, input) in edge_cases.iter().enumerate() {
            ...
            assert!(
                output.len() >= 2,
                "Edge case {} should have minimum encoding overhead",
                i
            );
        }
    ...
    fn test_reference_compliant_suffix_array_vs_hash_table() -> Result<()> {
        ...
        assert!(
            sa_output.len() >= 4,
            "Suffix array compression should have minimum overhead"
        );
        assert!(
            ht_output.len() >= 4,
            "Hash table compression should have minimum overhead"
        );

        println!(
            "Suffix array compression: {} -> {} bytes (ratio: {:.3})",
            input.len(),
            sa_output.len(),
            sa_stats.compression_ratio
        );
        println!(
            "Hash table compression: {} -> {} bytes (ratio: {:.3})",
            input.len(),
            ht_output.len(),
            ht_stats.compression_ratio
        );
