...
impl<W: Write> ReferenceEncoder<W> {
    ...
    pub fn encode_near_short(&mut self, distance: usize, length: usize) -> Result<()> {
        ...
        let encoded =
            DzType::NearShort.as_u8() | (((length - 2) << 3) as u8) | (((distance - 2) << 5) as u8);
        self.writer.write_all(&[encoded])?;
        Ok(())
    }
    ...
    pub fn encode_global(
        ...
        if length <= g_max_short_len && enc_len <= 31 {
            ...
            if g_offset_bits < 24 {
                let offset_encoded =
                    (dict_position << (24 - g_offset_bits)) | ((enc_len >> 5) as u32);
                write_uint_bytes(&mut self.writer, offset_encoded, 3)?;
            } else if g_offset_bits > 24 {
                let offset_encoded =
                    (dict_position << (32 - g_offset_bits)) | ((enc_len >> 5) as u32);
            ...
            if g_offset_bits < 24 {
                let offset_encoded =
                    (dict_position << (24 - g_offset_bits)) | (0x00FFFFFF >> g_offset_bits);
                write_uint_bytes(&mut self.writer, offset_encoded, 3)?;
            } else if g_offset_bits > 24 {
                let offset_encoded =
                    (dict_position << (32 - g_offset_bits)) | (0xFFFFFFFF >> g_offset_bits);
                write_uint_bytes(&mut self.writer, offset_encoded, 4)?;
            } else {
                // g_offset_bits == 24
                write_uint_bytes(&mut self.writer, dict_position, 3)?;
...
pub fn compress_record_reference<W: Write>(
    ...
    if use_suffix_array_local_match {
        compress_record_with_suffix_array(
            input_data,
            writer,
            global_dictionary,
            g_offset_bits,
        ...
        compress_record_with_hash_table(
            input_data,
            writer,
            global_dictionary,
            g_offset_bits,
...
fn compress_record_with_suffix_array<W: Write>(
    ...
    while pos < input_data.len() {
        ...
        if let Some((distance, length)) =
            find_suffix_array_match(input_data, pos, &suffix_array, 30)
            && length >= 2
        {
            let meta = get_back_ref_encoding_meta(distance, length);
            let cost = meta.len as i32;
...
fn find_suffix_array_match(
    ...
    for i in search_start..search_end {
        ...
        if distance > 0 && distance < (1 << 24) {
            ...
            if length >= 2
                && (best_match.is_none()
                    || length > best_match.expect("best_match set by condition").1)
            {
                best_match = Some((distance, length));
            }
        }
    ...
    for dict_pos in 0..dict_data.len() {
        ...
        if match_len >= 6 {
            ...
            if best_match.is_none()
                || match_len > best_match.expect("best_match set by condition").1
            {
...
mod tests {
    ...
    fn test_compress_record_reference_rle_pattern() {
        ...
        let result = compress_record_reference(input_data, &mut output, false, None, 0, 0);
    ...
    fn test_compress_record_various_compression_types() {
        ...
        let test_cases = vec![
            ...
            (
                b"hello world hello world hello".as_slice(),
                "Repeated phrase",
            ),
            (
    ...
    fn test_compression_type_boundaries() {
        ...
        assert_eq!(
            get_back_ref_encoding_meta(257, 33).dz_type,
            DzType::Far1Short
        );
        // Distance out of range
        assert_ne!(
            get_back_ref_encoding_meta(258, 33).dz_type,
            DzType::Far1Short
        );
        ...
        assert_eq!(
            get_back_ref_encoding_meta(258 + 65535, 33).dz_type,
            DzType::Far2Short
        );
        // Length out of range
        assert_ne!(
            get_back_ref_encoding_meta(258, 34).dz_type,
            DzType::Far2Short
        );

        // Far2Long boundaries: distance <= 65535, len >= 34
        assert_eq!(
            get_back_ref_encoding_meta(65535, 34).dz_type,
            DzType::Far2Long
        );
        assert_eq!(
            get_back_ref_encoding_meta(1000, 100).dz_type,
            DzType::Far2Long
        );
        // Distance out of range should be Far3Long
        assert_eq!(
            get_back_ref_encoding_meta(65536, 34).dz_type,
            DzType::Far3Long
        );

        // Far3Long fallback case
        assert_eq!(
            get_back_ref_encoding_meta(1000000, 35).dz_type,
            DzType::Far3Long
        );
        assert_eq!(
            get_back_ref_encoding_meta(1000000, 100).dz_type,
            DzType::Far3Long
        );
    }
    ...
    fn test_all_encoder_types() {
        ...
        // Test Far3Long encoding - short length
        {
            ...
            let distance =
                u32::from_le_bytes([distance_bytes[0], distance_bytes[1], distance_bytes[2], 0]);
    ...
    fn test_edge_case_compression_scenarios() {
        ...
        let mut output = Vec::new();
        let result =
            compress_record_reference(input_data, &mut output, false, Some(dictionary), 20, 32);
        assert!(result.is_ok());
        assert!(!output.is_empty());
