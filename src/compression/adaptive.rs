...
impl AdaptiveCompressor {
        ...
        *operation_count += 1;
        ...
        if best_algorithm != self.current_algorithm {
            ...
            if improvement > self.config.switch_threshold {
                ...
                log::info!("Would switch to algorithm: {:?} (improvement: {:.2}%)", 
                          best_algorithm, improvement * 100.0);
            }
        }
    ...
    fn find_best_algorithm(&self, sample_data: &[u8]) -> Result<Algorithm> {
        ...
        for measurement in recent_window {
            ...
            algorithm_scores.entry(measurement.algorithm)
        ...
        for (algorithm, scores) in algorithm_scores {
            if scores.len() >= 5 { // Minimum samples for confidence
    ...
    fn estimate_improvement(&self, new_algorithm: Algorithm) -> f64 {
        ...
        let current_scores: Vec<f64> = history.iter()
            .rev()
        ...
        let new_scores: Vec<f64> = history.iter()
            .rev()
            .take(self.config.learning_window)
    ...
    fn update_profile(&self, data_type: &str, measurement: &PerformanceMeasurement) {
        ...
        let profile = profiles.entry(data_type.to_string())
            .or_insert_with(|| CompressionProfile::new(data_type.to_string(), measurement.algorithm));
        ...
        let score = measurement.score(&self.requirements);
        if score > 0.5 { // Arbitrary threshold for good performance
