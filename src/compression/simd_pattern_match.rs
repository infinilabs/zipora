...
use crate::compression::dict_zip::compression_types::{CompressionType, Match};
use crate::compression::dict_zip::matcher::{MatcherConfig, PatternMatcher};
...
impl SimdPatternStats {
    ...
    pub fn update_with_result(&mut self, result: &SimdMatchResult) {
        ...
        if result.length > 0 {
            ...
            let total_length = self.avg_pattern_length * (self.successful_matches - 1) as f64
                + result.length as f64;
            self.avg_pattern_length = total_length / self.successful_matches as f64;

            let total_quality =
                self.avg_match_quality * (self.successful_matches - 1) as f64 + result.quality;
...
impl SimdPatternMatcher {
    ...
    fn find_matches_single_sse(
        ...
        match self.simd_tier {
            ...
            SimdPatternTier::Scalar => self.scalar_pattern_search(input, pattern, max_matches),
        }
    }
    ...
    pub fn find_parallel_pattern_matches(
        ...
        if !parallel_mode.is_supported(self.cpu_features) {
            ...
            return self.find_parallel_pattern_matches(
                input,
                patterns,
    ...
    fn find_patterns_x2(
        ...
        let right_results =
            self.find_patterns_x1(input, right_patterns, max_matches_per_pattern)?;
    ...
    pub fn find_compression_aware_matches(
        ...
        for (input_pos, dict_pos) in matches {
            let base_match =
                self.create_compression_aware_match(pattern, dict_pos, compression_type)?;
            let mut result = SimdMatchResult::new(
                base_match,
                input_pos,
                dict_pos,
    ...
    fn create_compression_aware_match(
        ...
        match compression_type {
            CompressionType::Literal => Match::literal(length.min(255) as u8),
            CompressionType::Global => {
            ...
            CompressionType::Far2Long => {
                let distance = dict_position as u32;
                Match::far2_long(
                    distance.try_into().unwrap_or(u16::MAX),
                    length.min(65535) as u16,
                )
            }
...
static GLOBAL_SIMD_PATTERN_MATCHER: std::sync::OnceLock<SimdPatternMatcher> =
...
mod tests {
    ...
    fn test_simd_pattern_matcher_creation() {
        ...
        assert!(matches!(
            matcher.simd_tier(),
            SimdPatternTier::Scalar | SimdPatternTier::Sse42 | SimdPatternTier::Avx2
        ));
    }
    ...
    fn test_parallel_mode_support() {
        ...
        println!(
            "X2 supported: {}",
            ParallelMode::X2.is_supported(cpu_features)
        );
        println!(
            "X4 supported: {}",
            ParallelMode::X4.is_supported(cpu_features)
        );
        println!(
            "X8 supported: {}",
            ParallelMode::X8.is_supported(cpu_features)
        );
    }
    ...
    fn test_multi_level_search_strategy() {
        ...
        let short_results = matcher
            .find_pattern_matches(&input, short_pattern, 5)
            .unwrap();
        assert!(!short_results.is_empty());

        // Test medium pattern (17-35 bytes)
        let medium_pattern = b"abcdefghijklmnopqrstuvwxyz";
        let medium_results = matcher
            .find_pattern_matches(&input, medium_pattern, 5)
            .unwrap();
        ...
        let large_results = matcher
            .find_pattern_matches(&input, large_pattern, 5)
            .unwrap();
    ...
    fn test_parallel_processing_variants() {
        ...
        // Test X1 processing
        let x1_results = matcher
            .find_parallel_pattern_matches(input, &patterns, 5, ParallelMode::X1)
            .unwrap();
        ...
        // Test X2 processing
        let x2_results = matcher
            .find_parallel_pattern_matches(input, &patterns, 5, ParallelMode::X2)
            .unwrap();
        assert_eq!(x2_results.len(), 4);

        // Test X4 processing
        let x4_results = matcher
            .find_parallel_pattern_matches(input, &patterns, 5, ParallelMode::X4)
            .unwrap();
        assert_eq!(x4_results.len(), 4);

        // Test X8 processing
        let x8_results = matcher
            .find_parallel_pattern_matches(input, &patterns, 5, ParallelMode::X8)
            .unwrap();
    ...
    fn test_compression_aware_matching() {
        ...
        let literal_results = matcher
            .find_compression_aware_matches(input, literal_pattern, CompressionType::Literal, 5)
            .unwrap();
        assert!(!literal_results.is_empty());

        // Test RLE matching
        let rle_pattern = b"aaaaaaaaaa";
        let rle_results = matcher
            .find_compression_aware_matches(input, rle_pattern, CompressionType::RLE, 5)
            .unwrap();
        assert!(!rle_results.is_empty());

        // Test global matching
        let global_pattern = b"global";
        let global_results = matcher
            .find_compression_aware_matches(input, global_pattern, CompressionType::Global, 5)
            .unwrap();
    ...
    fn test_large_pattern_fallback() {
        ...
        println!(
            "Large pattern fallback test completed with {} results",
            results.len()
        );
    }
    ...
    fn test_compression_type_match_creation() {
        ...
        for (compression_type, dict_pos) in &test_cases {
            let result =
                matcher.create_compression_aware_match(pattern, *dict_pos, *compression_type);
            match result {
                Ok(_) => {
                    println!(
                        "Successfully created match for compression type: {:?}",
                        compression_type
                    );
                }
                Err(e) => {
                    println!(
                        "Error creating match for compression type: {:?} with dict_pos: {}: {:?}",
                        compression_type, dict_pos, e
                    );
                    panic!(
                        "Failed to create match for compression type: {:?} with dict_pos: {} - Error: {:?}",
                        compression_type, dict_pos, e
                    );
                }
            }
