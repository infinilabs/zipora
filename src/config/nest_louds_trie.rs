...
impl Config for NestLoudsTrieConfig {
    fn validate(&self) -> Result<()> {
        ...
        if self.max_fragment_length > 0
            && self.max_fragment_length < self.min_fragment_length as i32
        {
            errors.push(ValidationError::new(
                "max_fragment_length",
        ...
        if self.core_str_compression_level > 22 {
            errors.push(
                ...
                .with_suggestion(
                    "typical values: 1-6 for speed, 7-15 for balance, 16-22 for compression",
                ),
            );
        }
    ...
    fn from_env_with_prefix(prefix: &str) -> Result<Self> {
        ...
        config.max_fragment_length = parse_env_var(
            &format!("{}TRIE_MAX_FRAGMENT_LENGTH", prefix),
            config.max_fragment_length,
        );
        config.min_fragment_length = parse_env_var(
            &format!("{}TRIE_MIN_FRAGMENT_LENGTH", prefix),
            config.min_fragment_length,
        );
        config.min_link_str_length = parse_env_var(
            &format!("{}TRIE_MIN_LINK_STR_LENGTH", prefix),
            config.min_link_str_length,
        );

        // Compression settings
        config.core_str_compression_level = parse_env_var(
            &format!("{}TRIE_COMPRESSION_LEVEL", prefix),
            config.core_str_compression_level,
        );
        config.sa_fragment_min_freq = parse_env_var(
            &format!("{}TRIE_SA_FRAGMENT_MIN_FREQ", prefix),
            config.sa_fragment_min_freq,
        );
        config.compression_min_length = parse_env_var(
            &format!("{}TRIE_COMPRESSION_MIN_LENGTH", prefix),
            config.compression_min_length,
        );
        ...
        config.initial_pool_size = parse_env_var(
            &format!("{}TRIE_INITIAL_POOL_SIZE", prefix),
            config.initial_pool_size,
        );
        config.load_factor =
            parse_env_var(&format!("{}TRIE_LOAD_FACTOR", prefix), config.load_factor);

        // Performance tuning
        config.is_input_sorted = parse_env_bool(
            &format!("{}TRIE_INPUT_SORTED", prefix),
            config.is_input_sorted,
        );
        config.nest_scale = parse_env_var(&format!("{}TRIE_NEST_SCALE", prefix), config.nest_scale);
        config.enable_queue_compression = parse_env_bool(
            &format!("{}TRIE_QUEUE_COMPRESSION", prefix),
            config.enable_queue_compression,
        );
        config.use_mixed_core_link = parse_env_bool(
            &format!("{}TRIE_MIXED_CORE_LINK", prefix),
            config.use_mixed_core_link,
        );
        config.speedup_nest_trie_build = parse_env_bool(
            &format!("{}TRIE_SPEEDUP_BUILD", prefix),
            config.speedup_nest_trie_build,
        );

        // Advanced features
        config.debug_level =
            parse_env_var(&format!("{}TRIE_DEBUG_LEVEL", prefix), config.debug_level);
        config.enable_statistics = parse_env_bool(
            &format!("{}TRIE_ENABLE_STATISTICS", prefix),
            config.enable_statistics,
        );
        config.enable_profiling = parse_env_bool(
            &format!("{}TRIE_ENABLE_PROFILING", prefix),
            config.enable_profiling,
        );
        config.max_bfs_depth = parse_env_var(
            &format!("{}TRIE_MAX_BFS_DEPTH", prefix),
            config.max_bfs_depth,
        );
        config.cache_frequency_threshold = parse_env_var(
            &format!("{}TRIE_CACHE_FREQ_THRESHOLD", prefix),
            config.cache_frequency_threshold,
        );
        config.enable_adaptive_optimization = parse_env_bool(
            &format!("{}TRIE_ADAPTIVE_OPTIMIZATION", prefix),
            config.enable_adaptive_optimization,
        );
        config.parallel_threads = parse_env_var(
            &format!("{}TRIE_PARALLEL_THREADS", prefix),
            config.parallel_threads,
        );
        config.enable_mmap_operations = parse_env_bool(
            &format!("{}TRIE_ENABLE_MMAP", prefix),
            config.enable_mmap_operations,
        );
        config.node_cache_size = parse_env_var(
            &format!("{}TRIE_NODE_CACHE_SIZE", prefix),
    ...
    fn load_from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        #[cfg(feature = "serde")]
        {
            ...
            let config: Self = serde_json::from_str(&content).map_err(|e| {
                ZiporaError::configuration(format!("Failed to parse config file: {}", e))
            })?;
...
impl NestLoudsTrieConfig {
    ...
    /// Set nest level.
    pub fn nest_level(mut self, v: u8) -> Self {
        self.nest_level = v;
        self
    }
    /// Set compression level.
    pub fn compression_level(mut self, v: u8) -> Self {
        self.core_str_compression_level = v;
        self
    }
    /// Set max fragment length.
    pub fn max_fragment_length(mut self, v: i32) -> Self {
        self.max_fragment_length = v;
        self
    }
    /// Set min fragment length.
    pub fn min_fragment_length(mut self, v: u32) -> Self {
        self.min_fragment_length = v;
        self
    }
    /// Set compression algorithm.
    pub fn compression_algorithm(mut self, v: CompressionAlgorithm) -> Self {
        self.compression_algorithm = v;
        self
    }
    /// Set queue compression.
    pub fn enable_queue_compression(mut self, v: bool) -> Self {
        self.enable_queue_compression = v;
        self
    }
    /// Set temp directory.
    pub fn temp_directory(mut self, v: &str) -> Self {
        self.temp_directory = v.to_string();
        self
    }
    ...
    /// Enable profiling.
    pub fn enable_profiling(mut self, v: bool) -> Self {
        self.enable_profiling = v;
        self
    }
    /// Set parallel threads.
    pub fn parallel_threads(mut self, v: usize) -> Self {
        self.parallel_threads = v as u32;
        self
    }
    /// Set optimization flags.
    pub fn optimization_flags(mut self, v: OptimizationFlags) -> Self {
...
impl serde::Serialize for NestLoudsTrieConfig {
        ...
        S: serde::Serializer,
    {
        ...
        state.serialize_field(
            "core_str_compression_level",
            &self.core_str_compression_level,
        )?;
        ...
        state.serialize_field(
            "best_delimiters",
            &self.best_delimiters.iter().cloned().collect::<Vec<u8>>(),
        )?;
                ...
                V: MapAccess<'de>,
            {
                ...
                while let Some(key) = map.next_key::<String>()? {
                    match key.as_str() {
                        ...
                        "core_str_compression_level" => {
                            config.core_str_compression_level = map.next_value()?
                        }
                        "sa_fragment_min_freq" => config.sa_fragment_min_freq = map.next_value()?,
                        "compression_min_length" => {
                            config.compression_min_length = map.next_value()?
                        }
                        "optimization_flags" => {
                        ...
                        "enable_queue_compression" => {
                            config.enable_queue_compression = map.next_value()?
                        }
                        "use_mixed_core_link" => config.use_mixed_core_link = map.next_value()?,
                        "speedup_nest_trie_build" => {
                            config.speedup_nest_trie_build = map.next_value()?
                        }
                        ...
                        "cache_frequency_threshold" => {
                            config.cache_frequency_threshold = map.next_value()?
                        }
                        "enable_adaptive_optimization" => {
                            config.enable_adaptive_optimization = map.next_value()?
                        }
                        "parallel_threads" => config.parallel_threads = map.next_value()?,
                        "enable_mmap_operations" => {
                            config.enable_mmap_operations = map.next_value()?
                        }
...
mod tests {
    ...
    fn test_serialization() {
        ...
        let deserialized: NestLoudsTrieConfig =
