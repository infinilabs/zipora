...
impl Config for CacheConfig {
    ...
    fn from_env_with_prefix(prefix: &str) -> Result<Self> {
        ...
        config.enable_prefetching = parse_env_bool(
            &format!("{}CACHE_PREFETCHING", prefix),
    ...
    fn save_to_file<P: AsRef<Path>>(&self, path: P) -> Result<()> {
        #[cfg(feature = "serde")]
        {
            let serialized = serde_json::to_string_pretty(self).map_err(|e| {
                ZiporaError::configuration(format!("Failed to serialize cache config: {}", e))
            })?;
            std::fs::write(path, serialized).map_err(|e| {
                ZiporaError::configuration(format!("Failed to write cache config file: {}", e))
            })?;
            Ok(())
        }
    ...
    fn load_from_file<P: AsRef<Path>>(path: P) -> Result<Self> {
        #[cfg(feature = "serde")]
        {
            let content = std::fs::read_to_string(path).map_err(|e| {
                ZiporaError::configuration(format!("Failed to read cache config file: {}", e))
            })?;
            let config: Self = serde_json::from_str(&content).map_err(|e| {
                ZiporaError::configuration(format!("Failed to parse cache config file: {}", e))
            })?;
            config.validate()?;
            Ok(config)
        }
