...
impl<T> AutoGrowCircularQueue<T> {
    ...
    fn grow_to(&mut self, new_capacity: usize) -> Result<()> {
        ...
        if self.len == 0 {
            ...
            if !self.buffer.is_null() {
                unsafe { dealloc(self.buffer as *mut u8, self.current_layout()); }
            }
    ...
    fn reorganize_after_realloc(&mut self, new_buffer: *mut T, _new_capacity: usize) -> Result<()> {
        ...
        let temp_layout = Layout::array::<T>(self.len).map_err(|_| ZiporaError::invalid_data("Layout error"))?;
    ...
    unsafe fn simd_copy_elements(&self, src: *const T, dst: *mut T, count: usize) {
        ...
        #[cfg(debug_assertions)]
        {
            ...
            debug_assert_eq!(src as usize % std::mem::align_of::<T>(), 0, "Source pointer not aligned");
            debug_assert_eq!(dst as usize % std::mem::align_of::<T>(), 0, "Destination pointer not aligned");
            ...
            debug_assert!(src_end <= dst_start || dst_end <= src_start, "Memory ranges overlap");
        }
    ...
    pub fn performance_stats(&self) -> AutoGrowQueueStats {
        AutoGrowQueueStats {
            ...
            length: self.len,
            utilization: if self.capacity > 0 { self.len as f64 / self.capacity as f64 } else { 0.0 },
    ...
    fn clone(&self) -> Self {
        ...
        if self.head <= self.tail {
            ...
            for i in self.head..self.tail {
                ...
                new_queue.push_back(value.clone()).expect("Clone should not fail with sufficient capacity");
            }
        } else {
    ...
    fn test_auto_queue_growth() -> Result<()> {
        ...
        for i in 0..10 {
            ...
            if popped != Some(i) {
                ...
                println!("Queue state: head={}, tail={}, len={}, capacity={}", 
                         queue.head, queue.tail, queue.len, queue.capacity());
            }
