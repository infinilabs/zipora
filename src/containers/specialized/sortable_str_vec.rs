...
impl SortableStrVec {
        ...
        S: AsRef<str>,
    {
        ...
        let estimated_arena_size: usize = iter.iter().map(|s| s.as_ref().len()).sum();
    ...
    pub fn push_str(&mut self, s: &str) -> Result<usize> {
        ...
        let packed = (offset as u64) | ((length as u64) << 40) | ((seq_id as u64) << 60);
    ...
    fn fast_comparison_sort(&mut self) -> Result<()> {
        #[cfg(debug_assertions)]
        {
            ...
            let mut sort_data: Vec<(usize, &[u8])> = self
                .sorted_indices
                .iter()
                .map(|&idx| {
    ...
    pub fn sort_by_length(&mut self) -> Result<()> {
        ...
        let entries = &self.entries;
        self.sorted_indices
            .sort_unstable_by_key(|&idx| entries[idx].length());
...
mod tests {
    ...
    fn test_radix_sort() {
        ...
        vec.push_str("the quick brown fox jumps over the lazy dog")
            .unwrap();
        vec.push_str("pack my box with five dozen liquor jugs")
            .unwrap();
