...
use super::lru_map::{
    EvictionCallback, LruMap, LruMapConfig, LruMapStatistics, NoOpEvictionCallback,
};
...
impl ConcurrentLruMapStatistics {
    ...
    pub fn hit_ratio(&self) -> f64 {
        let (total_hits, total_gets) =
            self.shard_stats
                .iter()
                .fold((0u64, 0u64), |(hits, gets), stats| {
                    (
    ...
    pub fn total_memory_usage(&self) -> usize {
        self.shard_stats
            .iter()
            .map(|stats| stats.memory_usage.load(Ordering::Relaxed))
            .sum()
    }
