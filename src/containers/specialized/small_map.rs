...
impl<K: PartialEq + Hash + Eq + 'static, V> SmallMap<K, V> {
    ...
    #[inline(always)]
    fn find_key_index(&self, key: &K, keys: &[MaybeUninit<K>; SMALL_MAP_THRESHOLD], len: usize) -> Option<usize> {
        ...
        match len {
            ...
            5..=8 => {
                ...
                if k0 == key { return Some(0); }
                if k1 == key { return Some(1); }
                if k2 == key { return Some(2); }
                if k3 == key { return Some(3); }
    ...
    #[cold]
    fn find_key_fallback(&self, key: &K, keys: &[MaybeUninit<K>; SMALL_MAP_THRESHOLD], len: usize) -> Option<usize> {
    ...
    pub fn insert(&mut self, key: K, value: V) -> Result<Option<V>> {
        match &mut self.storage {
            ...
            SmallMapStorage::Large(map) => {
                map.insert(key, value).map_err(|_| ZiporaError::invalid_data("Failed to insert into large map"))
            }
        }
    }
...
impl<'a, K, V> Iterator for SmallMapIter<'a, K, V> {
    ...
    fn next(&mut self) -> Option<Self::Item> {
        match self {
            SmallMapIter::Small { keys, values, index, len } => {
                if *index < *len {
...
trait OptimizedSearch {
    ...
    fn find_optimized(&self, keys: &[MaybeUninit<Self>; SMALL_MAP_THRESHOLD], len: usize) -> Option<usize>
    where
...
impl OptimizedSearch for i32 {
    ...
    fn find_optimized(&self, keys: &[MaybeUninit<Self>; SMALL_MAP_THRESHOLD], len: usize) -> Option<usize> {
        ...
        unsafe {
            ...
            let u32_keys = std::mem::transmute::<&[MaybeUninit<i32>; SMALL_MAP_THRESHOLD], &[MaybeUninit<u32>; SMALL_MAP_THRESHOLD]>(keys);
...
impl<V> SmallMap<u8, V> {
    ...
    #[inline(always)]
    fn find_key_index_simd(&self, key: &u8, keys: &[MaybeUninit<u8>; SMALL_MAP_THRESHOLD], len: usize) -> Option<usize> {
