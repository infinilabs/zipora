    ...
    pub fn validate(&self) -> Result<()> {
        ...
        if !self.initial_hash_capacity.is_power_of_two() {
            return Err(ZiporaError::invalid_parameter(
                "Initial hash capacity must be power of 2",
            ));
        }
...
impl LruList {
    ...
    fn insert_head<K, V>(&self, nodes: &mut [LruNode<K, V>], node_idx: u32) {
        ...
        nodes[node_idx as usize]
            .prev
            .store(INVALID_NODE, Ordering::Relaxed);
        nodes[node_idx as usize]
            .next
            .store(old_head, Ordering::Relaxed);

        if old_head != INVALID_NODE {
            nodes[old_head as usize]
                .prev
                .store(node_idx, Ordering::Relaxed);
        } else {
    ...
    E: EvictionCallback<K, V>,
{
    ...
    fn allocate_node(&self) -> Result<u32> {
        ...
        free_nodes
            .pop()
            .ok_or_else(|| ZiporaError::out_of_memory(0))
    }
