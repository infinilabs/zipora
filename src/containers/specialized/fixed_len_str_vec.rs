...
impl<const N: usize> FixedLenStrVec<N> {
    ...
    pub fn push(&mut self, s: &str) -> Result<()> {
        ...
        if s_bytes.len() > N {
            return Err(ZiporaError::invalid_data(format!(
                "String length {} exceeds fixed length {}",
                s_bytes.len(),
                N
            )));
        }
    ...
    pub fn memory_savings_vs_vec_string(&self) -> (usize, usize, f64) {
        ...
        (
            memory_info.vec_string_equivalent_bytes,
            memory_info.total_bytes,
            memory_info.memory_ratio,
        )
    }
    ...
    fn update_stats(&mut self) {
        ...
        self.stats.memory_saved_vs_vec_string = vec_string_size.saturating_sub(our_size);
    }
