...
impl FseEncoder {
    ...
    fn compress_parallel(&mut self, data: &[u8], num_blocks: usize) -> Result<Vec<u8>> {
        ...
        let table = self
            .table
            .as_ref()
            .expect("FSE table must be initialized")
            .clone();
        ...
        for handle in handles {
            match handle.join() {
                ...
                Err(_) => {
                    return Err(ZiporaError::invalid_data(
                        "Thread panicked during compression",
                    ));
                }
            }
        }
...
impl FseDecoder {
    ...
    fn decompress_single(&mut self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        for _i in 0..original_size {
            ...
            if let Some(renorm_state) =
                table.renormalize_decode(state, compressed_data, &mut byte_pos)
            {
                state = renorm_state;
            } else {
                return Err(ZiporaError::invalid_data(
