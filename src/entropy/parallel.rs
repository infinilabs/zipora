...
use crate::entropy::{
    EntropyStats,
    fse::{FseConfig, FseEncoder},
    huffman::{HuffmanDecoder, HuffmanEncoder, HuffmanTree},
    rans::{ParallelX2, ParallelX4, ParallelX8, Rans64Encoder},
};
...
impl<P: ParallelVariant> ParallelHuffmanEncoder<P> {
    ...
    pub fn encode(&mut self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        if data.len() < self.config.min_parallel_size {
            ...
            println!(
                "Parallel Huffman ({}) - Encoded {} bytes to {} bytes in {:.2}ms (single stream)",
                P::NAME,
                data.len(),
                compressed.len(),
                elapsed.as_millis()
        ...
        println!(
            "Parallel Huffman ({}) - Encoded {} bytes to {} bytes in {:.2}ms",
            P::NAME,
            data.len(),
            compressed.len(),
            elapsed.as_millis()
    ...
    fn split_data_into_blocks<'a>(&self, data: &'a [u8]) -> Vec<&'a [u8]> {
        let block_size = if self.config.adaptive_blocks {
            ...
            optimal_size
                .max(self.config.block_size / 4)
                .min(self.config.block_size * 4)
        } else {
    ...
    pub fn decode(&mut self, data: &[u8], output_length: usize) -> Result<Vec<u8>> {
        if self.decoders.is_empty() {
            return Err(ZiporaError::invalid_data(
                "Decoder not initialized with tree",
            ));
        }
    ...
    pub fn encode_adaptive(&mut self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        match result {
            Ok(compressed) => {
                ...
                println!(
                    "Adaptive encoding: {} {} - {:.2}x compression in {:.2}ms",
                    algorithm,
                    variant,
                    ratio,
                    elapsed.as_millis()
...
impl Default for AdaptiveParallelEncoder {
    fn default() -> Self {
        ...
        Self::new().unwrap_or_else(|e| {
            panic!(
                "AdaptiveParallelEncoder creation failed in Default: {}. \
...
impl ParallelBenchmark {
    ...
    fn record_result(
        ...
        let throughput_mbps =
            (self.test_data.len() as f64 / 1024.0 / 1024.0) / elapsed.as_secs_f64();
    ...
    pub fn get_best_throughput(&self) -> Option<&BenchmarkResult> {
        self.results.values().max_by(|a, b| {
            a.throughput_mbps
...
mod tests {
    ...
    fn test_parallel_huffman_x2() -> Result<()> {
        ...
        let test_data =
            b"Parallel Huffman encoding test data with sufficient length for parallel processing."
                .repeat(100);
    ...
    fn test_parallel_benchmark() -> Result<()> {
        ...
        test_data.extend_from_slice(
            "More varied content for comprehensive testing. "
                .repeat(500)
                .as_bytes(),
        );
...
mod bench_tests {
    ...
    fn bench_parallel_variants_performance() -> Result<()> {
        ...
        println!(
            "x2 variant: {:.2} MB/s, {} bytes compressed",
            throughput_x2,
            compressed_x2.len()
        );
        println!(
            "x4 variant: {:.2} MB/s, {} bytes compressed",
            throughput_x4,
            compressed_x4.len()
        );
        println!(
            "x8 variant: {:.2} MB/s, {} bytes compressed",
            throughput_x8,
            compressed_x8.len()
        );
    ...
    fn bench_adaptive_selection_accuracy() -> Result<()> {
        ...
        let test_cases = vec![
            ...
            (
                (0..=255u8).cycle().take(100000).collect(),
                "high_entropy_large",
            ),
            (
                "Mixed content with patterns and randomness"
                    .repeat(1000)
                    .as_bytes()
                    .to_vec(),
                "mixed_medium",
