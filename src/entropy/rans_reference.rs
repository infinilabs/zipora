...
impl ReferenceRans {
    ...
    pub fn encode(&self, data: &[u8]) -> Result<Vec<u8>> {
        ...
        for (i, &symbol) in data.iter().rev().enumerate() {
            ...
            println!(
                "  Max state for freq {}: {}, current state: {}",
                freq, max_state, state
            );

            while (state as u64) >= max_state {
                let byte_out = (state & 0xFF) as u8;
                println!(
                    "  Renormalizing: outputting byte {}, state {} -> {}",
                    byte_out,
                    state,
                    state >> 8
                );
            ...
            let new_state_u64 =
                ((state_u64 / freq_u64) * total_freq_u64) + (state_u64 % freq_u64) + cumfreq_u64;
    ...
    pub fn decode(&self, encoded: &[u8], length: usize) -> Result<Vec<u8>> {
        ...
        for i in 0..length {
            ...
            while state < RANS_BYTE_L {
                ...
                println!(
                    "  Renormalizing: reading byte {} at pos {}, state {} -> {}",
