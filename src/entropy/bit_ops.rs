...
impl BitOps {
    ...
    pub fn parallel_bit_extract_bmi2(&self, source: u64, field_masks: &[u64]) -> Vec<u64> {
        if self.config.enable_bmi2
            && self.config.enable_compression_optimizations
    ...
    #[inline]
    pub fn decode_variable_length_bmi2(
        &self,
        bit_stream: u64,
        start_bit: u32,
        length: u32,
        ...
        if self.config.enable_bmi2
            && self.config.enable_variable_length_decoding
            && self.features.has_bmi2
        {
            #[cfg(target_arch = "x86_64")]
            {
                let extracted =
                    Bmi2CompressionOps::extract_entropy_field(bit_stream, start_bit, length);
                return Ok(extracted);
            }
        }
    ...
    #[inline]
    pub fn extract_huffman_symbols_bmi2(
        &self,
        packed_symbols: u64,
        symbol_masks: &[u64],
    ) -> Vec<u32> {
        if self.config.enable_bmi2
            && self.config.enable_entropy_acceleration
            && self.features.has_bmi2
        {
...
impl CompressionBmi2Dispatcher {
    ...
    pub fn dispatch_entropy_extract(
        &self,
        bit_stream: u64,
        start_bit: u32,
        field_length: u32,
    ...
    pub fn dispatch_variable_length_decode(
        &self,
        packed_data: u64,
        symbol_masks: &[u64],
    ) -> Vec<u32> {
    ...
    pub fn dispatch_bit_stream_process(
        &self,
        data: &[u64],
        ...
        if self.capabilities.has_bmi2 && self.config.enable_compression_optimizations {
            match operation {
                CompressionOperation::PopCount => data
                    .iter()
                    .map(|&word| Bmi2RankOps::popcount_u64(word) as u64)
                    .collect(),
                CompressionOperation::LeadingZeros => data
                    .iter()
                    .map(|&word| Bmi2RankOps::leading_zeros(word) as u64)
                    .collect(),
                CompressionOperation::TrailingZeros => data
                    .iter()
                    .map(|&word| Bmi2RankOps::trailing_zeros(word) as u64)
                    .collect(),
                CompressionOperation::BitReverse => data
                    .iter()
                    .map(|&word| Bmi2BitOps::deposit_bits(word.reverse_bits(), u64::MAX))
                    .collect(),
            }
        } else {
            // Software fallback
            match operation {
                ...
                CompressionOperation::LeadingZeros => data
                    .iter()
                    .map(|&word| word.leading_zeros() as u64)
                    .collect(),
                CompressionOperation::TrailingZeros => data
                    .iter()
                    .map(|&word| word.trailing_zeros() as u64)
                    .collect(),
...
mod tests {
    ...
    fn test_config_detection() {
        ...
        println!(
            ...
            config.enable_compression_optimizations
        );
        println!(
            "Entropy acceleration: {}",
            config.enable_entropy_acceleration
        );
        println!(
            "Variable-length decoding: {}",
            config.enable_variable_length_decoding
        );
    ...
    fn test_bmi2_variable_length_encoding() {
        ...
        let decoded = bit_ops
            .decode_variable_length_bmi2(bit_stream, 0, 8)
            .unwrap();
        assert_eq!(decoded, 0x12);

        let decoded2 = bit_ops
            .decode_variable_length_bmi2(bit_stream, 8, 8)
            .unwrap();
    ...
    fn test_entropy_huffman_extraction() {
        ...
        let symbols = entropy_ops
            .bit_ops()
            .extract_huffman_symbols_bmi2(packed_symbols, &symbol_masks);
    ...
    fn test_compression_dispatcher() {
        ...
        let popcounts =
            dispatcher.dispatch_bit_stream_process(&data, CompressionOperation::PopCount);
    ...
    fn test_bmi2_bit_operations() {
        ...
        let masks = vec![
            0x000000000000FFFFu64,
            0x00000000FFFF0000u64,
            0x0000FFFF00000000u64,
        ];
