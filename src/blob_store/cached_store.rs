...
use super::BlobStore;
use crate::RecordId;
use crate::cache::{CacheBuffer, FileId, LruPageCache, PageCacheConfig};
...
impl<T: BlobStore> CachedBlobStore<T> {
    ...
    pub fn with_cache_and_strategy(
        inner: T,
        cache: Arc<LruPageCache>,
        strategy: CacheWriteStrategy,
    ) -> Result<Self> {
    ...
    fn get_blob_metadata(&self, id: RecordId) -> Result<Option<(u64, usize)>> {
        let metadata = self.blob_metadata.lock().map_err(|_| {
            crate::error::ZiporaError::invalid_data("Metadata lock poisoned".to_string())
        })?;
...
impl<T: BlobStore> BlobStore for CachedBlobStore<T> {
    ...
    fn put(&mut self, data: &[u8]) -> Result<RecordId> {
        ...
        // Update metadata
        {
            let mut metadata = self.blob_metadata.lock().map_err(|_| {
                crate::error::ZiporaError::invalid_data("Metadata lock poisoned".to_string())
            })?;
...
mod tests {
    ...
    fn test_write_strategies() {
        ...
        let cached_store =
            CachedBlobStore::with_write_strategy(inner1, config1, CacheWriteStrategy::WriteThrough)
                .unwrap();
        assert_eq!(
            cached_store.write_strategy(),
            CacheWriteStrategy::WriteThrough
        );
        ...
        let cached_store =
            CachedBlobStore::with_write_strategy(inner2, config2, CacheWriteStrategy::WriteBack)
                .unwrap();
        ...
        let cached_store =
            CachedBlobStore::with_write_strategy(inner3, config3, CacheWriteStrategy::WriteAround)
                .unwrap();
        assert_eq!(
            cached_store.write_strategy(),
            CacheWriteStrategy::WriteAround
    ...
    fn test_write_through_operations() {
        ...
        let mut cached_store =
            CachedBlobStore::with_write_strategy(inner, config, CacheWriteStrategy::WriteThrough)
                .unwrap();
    ...
    fn test_write_back_operations() {
        ...
        let config = PageCacheConfig::performance_optimized();
        let mut cached_store =
            CachedBlobStore::with_write_strategy(inner, config, CacheWriteStrategy::WriteBack)
                .unwrap();
