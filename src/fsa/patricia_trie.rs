...
impl PatriciaTrie {
    ...
    fn find_node(&self, key: &[u8]) -> Option<usize> {
        ...
        loop {
            ...
            if remaining_key.is_empty() {
                ...
                return if node.is_final { Some(current_idx) } else { None };
            }
    ...
    fn collect_keys_with_prefix(&self, node_idx: usize, current_path: &[u8], prefix: &[u8], results: &mut Vec<Vec<u8>>) {
    ...
    fn collect_stats(&self, node_idx: usize, depth: usize, stats: &mut (usize, usize, usize, usize)) {
        let node = &self.nodes[node_idx];
...
impl FiniteStateAutomaton for PatriciaTrie {
    ...
    fn transitions(&self, state: StateId) -> Box<dyn Iterator<Item = (u8, StateId)> + '_> {
        if let Some(node) = self.nodes.get(state as usize) {
            let iter = node.children.iter().map(|(&byte, &idx)| (byte, idx as StateId));
...
impl StatisticsProvider for PatriciaTrie {
    fn stats(&self) -> TrieStats {
        ...
        let edge_memory: usize = self.nodes.iter()
            .map(|node| node.edge_label.len())
            .sum();
        let children_memory: usize = self.nodes.iter()
...
impl PrefixIterable for PatriciaTrie {
    fn iter_prefix(&self, prefix: &[u8]) -> Box<dyn Iterator<Item = Vec<u8>> + '_> {
        ...
        Box::new(PatriciaTriePrefixIterator {
            results,
            index: 0,
        })
    }
