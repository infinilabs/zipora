...
pub use zipora_trie::{
    BitVectorType, CompressionStrategy, RankSelectType, StorageStrategy, TrieStrategy, ZiporaTrie,
    ZiporaTrieConfig, ZiporaTrieMap,
};

// Strategy traits for advanced configuration
pub use strategy_traits::{
    AlgorithmStats, CompressionStats, CompressionStrategy as CompressionStrategyTrait,
...
pub use double_array::{
    DoubleArrayTrie, DoubleArrayTrieCursor, DoubleArrayTrieMap, MapValue, RangeIter,
};
...
pub mod nested_louds_trie {
    ...
    impl Default for NestingConfig {
        fn default() -> Self {
            Self {
                max_levels: 3,
                fragment_compression_ratio: 0.5,
    ...
    impl<T> NestedLoudsTrie<T> {
        pub fn new() -> Result<Self> {
            Ok(Self {
                trie: ZiporaTrie::with_config(ZiporaTrieConfig::space_optimized()),
                _marker: PhantomData,
            })
        }
        pub fn with_config(config: NestingConfig) -> Result<Self> {
            ...
            tc.trie_strategy = TrieStrategy::Louds {
                nesting_levels: config.max_levels,
                fragment_compression: true,
                adaptive_backends: true,
                cache_aligned: config.cache_optimization,
            };
            Ok(Self {
                trie: ZiporaTrie::with_config(tc),
                _marker: PhantomData,
            })
        }
        ...
        pub fn performance_stats(&self) -> NestedTrieStats {
            ...
            NestedTrieStats {
                key_count: s.num_keys,
                total_memory: s.memory_usage,
                num_states: s.num_states,
                num_keys: s.num_keys,
                num_transitions: s.num_transitions,
                max_depth: s.max_depth,
                avg_depth: s.avg_depth,
                memory_usage: s.memory_usage,
                bits_per_key: if s.num_keys > 0 {
                    (s.memory_usage * 8) as f64 / s.num_keys as f64
                } else {
                    0.0
                },
            }
    ...
    impl<T> Trie for NestedLoudsTrie<T> {
        fn insert(&mut self, key: &[u8]) -> Result<crate::StateId> {
            self.trie.insert(key)?;
            Ok(0)
        }
        ...
        fn lookup(&self, key: &[u8]) -> Option<crate::StateId> {
            if self.contains(key) { Some(0) } else { None }
        }
        fn len(&self) -> usize {
            self.trie.len()
        }
...
pub use nested_louds_trie::{
    FragmentStats, NestedLoudsTrie, NestedTrieStats, NestingConfig, NestingLevel,
};
...
pub mod compressed_sparse_trie {
    ...
    impl CompressedSparseTrie {
        pub fn new(_level: ConcurrencyLevel) -> Result<Self> {
            Ok(Self {
                trie: ZiporaTrie::with_config(ZiporaTrieConfig::sparse_optimized()),
            })
        }
        pub fn with_memory_pool(
            _level: ConcurrencyLevel,
            _pool: Arc<SecureMemoryPool>,
        ) -> Result<Self> {
            Self::new(_level)
        }
        pub fn insert(&mut self, key: &[u8]) -> Result<()> {
