...
impl<T> Future for FiberHandle<T> {
    ...
    fn poll(mut self: Pin<&mut Self>, cx: &mut Context<'_>) -> Poll<Self::Output> {
        match Pin::new(&mut self.inner).poll(cx) {
            ...
            Poll::Ready(Err(e)) => Poll::Ready(Err(ToplingError::configuration(&format!("fiber join error: {}", e)))),
...
impl FiberPool {
    ...
    pub fn new(config: FiberPoolConfig) -> Result<Self> {
        ...
        stats.active_workers.store(config.initial_workers, Ordering::Relaxed);
    ...
    where
        ...
        R: Send + 'static,
    {
        let handles: Vec<_> = iter.into_iter()
            .map(|item| {
                let f = f.clone();
    ...
    pub async fn shutdown(&self) -> Result<()> {
        ...
        let semaphore = self.semaphore.clone();
        let _permits = semaphore.acquire_many(self.config.max_fibers as u32).await
...
mod tests {
    ...
    async fn test_parallel_reduce() {
        ...
        let result = pool.parallel_reduce(input, 0, |acc, x| Ok(acc + x)).await.unwrap();
        assert_eq!(result, 15);
    }
