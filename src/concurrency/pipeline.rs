...
pub trait PipelineStage<T, U>: Send + Sync + 'static {
    ...
    fn process_batch(&self, inputs: Vec<T>) -> Pin<Box<dyn Future<Output = Result<Vec<U>>> + Send + '_>>
    where
        T: Send + 'static,
        U: Send + 'static,
        Self: Sized,
...
mod tests {
    ...
    async fn test_batch_collector_size_based_batching() {
        ...
        for i in 0..4 {
            ...
            assert!(result.is_none(), "Should not return batch before size limit");
    ...
    async fn test_batch_map_stage_with_batch_function() {
        ...
        let stage = BatchMapStage::with_batch_support(
            ...
            |batch: Vec<i32>| { // Batch: optimized version
    ...
    async fn test_batch_map_stage_with_max_concurrency() {
        let stage: BatchMapStage<_, fn(Vec<i32>) -> Result<Vec<i32>>> = 
            BatchMapStage::new("test".to_string(), |x: i32| Ok(x))
    ...
    async fn test_batch_map_stage_performance_comparison() {
        ...
        let _ = individual_stage.process_batch(inputs.clone()).await.unwrap();
    ...
    async fn test_end_to_end_pipeline_with_batching() {
        ...
        let stage = BatchMapStage::with_batch_support(
            ...
            |batch: Vec<i32>| { // Optimized batch processing
        ...
        let batch_results = pipeline.process_batch(batch_stage, batch_inputs).await.unwrap();
    ...
    async fn test_mixed_pipeline_batching_and_non_batching_stages() {
        ...
        let intermediate = pipeline.process_batch(batching_stage, vec![1, 2, 3]).await.unwrap();
        assert_eq!(intermediate, vec![3, 6, 9]);
        
        // Test second stage (non-batching) - should process individually
        let final_results = pipeline.process_batch(non_batching_stage, intermediate).await.unwrap();
    ...
    async fn test_pipeline_batch_collector_integration() {
        ...
        let handle = collector.start_timeout_checker(move |batch| {
            ...
            Box::pin(async move {
                ...
                let timeout_stage = BatchMapStage::with_batch_support(
                    ...
                    |batch: Vec<i32>| -> Result<Vec<i32>> { Ok(batch.into_iter().map(|x| x.pow(2)).collect()) }
                );
        ...
        for i in 1..=10 {
            if let Some(ready_batch) = collector.add(i).await.unwrap() {
                ...
                let results = pipeline.process_batch(immediate_stage, ready_batch).await.unwrap();
        ...
        if let Some(final_batch) = collector.flush().await.unwrap() {
            ...
            let results = pipeline.process_batch(final_stage, final_batch).await.unwrap();
            processed_batches.lock().await.push(results);
        }
    ...
    async fn test_pipeline_batching_with_errors() {
        ...
        let success_results = pipeline.process_batch(success_stage1, success_inputs).await.unwrap();
        assert_eq!(success_results, vec![2, 4, 6]);
        
        // Test error batch
        let error_inputs = vec![1, 0, 3];
        let error_result = pipeline.process_batch(error_prone_stage, error_inputs).await;
        assert!(error_result.is_err());
    ...
    async fn test_pipeline_timeout_scenarios() {
        ...
        let fast_stage = BatchMapStage::with_batch_support(
            ...
            |batch: Vec<i32>| -> Result<Vec<i32>> { Ok(batch.into_iter().map(|x| x * 2).collect()) }
        );
        ...
        let _timeout_result = short_timeout_pipeline.process_batch(slow_stage1, vec![1, 2]).await;
        
        // Test success with fast stage
        let success_result = long_timeout_pipeline.process_batch(fast_stage, vec![1, 2]).await.unwrap();
    ...
    async fn test_batch_vs_individual_throughput() {
        ...
        let individual_pipeline = PipelineBuilder::new()
            .enable_batching(false)
            .build();
        ...
        // Test individual processing
        let start_individual = Instant::now();
        let individual_results = individual_pipeline.process_batch(individual_stage, test_data.clone()).await.unwrap();
        let individual_elapsed = start_individual.elapsed();
        
        // Test batch processing
        let start_batch = Instant::now();
        let batch_results = batch_pipeline.process_batch(batch_stage, test_data).await.unwrap();
    ...
    async fn test_batch_collector_performance_under_load() {
        ...
        println!("Processed {} items in {:?} ({:.0} items/sec)", total_items, total_time, throughput);
    ...
    async fn test_pipeline_error_propagation() {
        ...
        assert!(individual_error.unwrap_err().to_string().contains("unlucky"));
        ...
        let batch_success = pipeline.process_batch(error_stage, vec![1, 2, 3]).await.unwrap();
    ...
    async fn test_pipeline_stage_timeout_configuration() {
        ...
        let success_result = long_timeout_pipeline.execute_single(slow_stage2, 5).await.unwrap();
        assert_eq!(success_result, 10);
    }
    ...
    async fn test_batch_map_stage_complex_error_scenarios() {
        ...
        let long_error = stage.process("this_is_a_very_long_string".to_string()).await;
        ...
        // Test batch errors
        let empty_batch_error = stage.process_batch(vec!["ok".to_string(), String::new()]).await;
        assert!(empty_batch_error.is_err());
        
        let long_batch_error = stage.process_batch(vec!["ok".to_string(), "very_long_string_here".to_string()]).await;
        assert!(long_batch_error.is_err());
        
        let success_batch = stage.process_batch(vec!["hello".to_string(), "world".to_string()]).await.unwrap();
