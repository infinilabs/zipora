...
fn test_basic_creation_and_configuration() {
    ...
    for level in levels {
        ...
        assert!(result.is_ok(), "Failed to create trie with level {:?}", level);
...
fn test_custom_memory_pool_creation() {
    ...
    let result = CompressedSparseTrie::with_memory_pool(
        ConcurrencyLevel::SingleThreadStrict,
        pool
    );
...
fn test_path_compression_efficiency() {
    ...
    assert!(stats.num_states < total_chars, 
...
async fn test_token_validation() {
    let trie1 = CompressedSparseTrie::new(ConcurrencyLevel::OneWriteMultiRead)
        .unwrap();
    let mut trie2 = CompressedSparseTrie::new(ConcurrencyLevel::OneWriteMultiRead)
        .unwrap();
...
fn test_large_dataset_performance() {
    ...
    println!("CSP Trie Performance - Insert: {:?}, Lookup: {:?}", 
        insert_duration, lookup_duration);
    
    // Performance should be reasonable
    assert!(insert_duration.as_secs() < 30, "Insert time too slow: {:?}", insert_duration);
...
fn test_memory_efficiency() {
    ...
    println!("Memory efficiency - Compression ratio: {:.2}", compression_ratio);
...
async fn test_concurrent_stress() {
    let trie = Arc::new(tokio::sync::Mutex::new(
        CompressedSparseTrie::new(ConcurrencyLevel::MultiWriteMultiRead)
            .unwrap()
    ));
        ...
        let handle = task::spawn(async move {
            ...
            if task_id % 2 == 0 {
                ...
                for i in 0..keys_per_task {
                    ...
                    trie_guard.insert_with_token(key.as_bytes(), &writer_token).unwrap();
                }
...
fn test_integration_with_memory_pool() {
    ...
    let mut trie = CompressedSparseTrie::with_memory_pool(
        ConcurrencyLevel::SingleThreadStrict,
        pool
    ).unwrap();
...
fn test_comparison_with_standard_trie() {
    ...
    let mut csp_trie = CompressedSparseTrie::new(ConcurrencyLevel::SingleThreadStrict)
        .unwrap();
...
async fn test_token_lifecycle_management() {
    ...
    for i in 0..10 {
        ...
        trie.insert_with_token(key.as_bytes(), &writer_token).unwrap();
