...
mod utility_tests {
    ...
    fn test_optimal_dict_size_calculation() {
        ...
        for (input_size, max_memory) in test_cases {
            ...
            println!(
                "Test case: input_size={}, max_memory={}, dict_size={}",
                input_size, max_memory, dict_size
            );
            ...
            if input_size > 0 {
                assert!(
                    dict_size <= input_size,
                    "Dictionary size {} exceeds input size {}",
                    dict_size,
                    input_size
                ); // Dict shouldn't be larger than input
            }
        }
    }
    ...
    fn test_compression_ratio_estimation() {
        ...
        for (entropy, repetitiveness, dict_ratio, (min_expected, max_expected)) in test_cases {
            ...
            assert!(
                ratio >= 0.1,
                "Ratio {} below minimum 0.1 for entropy={}, rep={}, dict={}",
                ratio,
                entropy,
                repetitiveness,
                dict_ratio
            ...
            assert!(
                ratio <= 1.0,
                "Ratio {} above maximum 1.0 for entropy={}, rep={}, dict={}",
                ratio,
                entropy,
                repetitiveness,
                dict_ratio
            ...
            if ratio < min_expected * 0.8 || ratio > max_expected * 1.2 {
                println!(
                    "Warning: Ratio {} outside expected range ({}, {}) for entropy={}, rep={}, dict={}",
    ...
    fn test_realistic_scenarios() {
        ...
        for (scenario, min_len, max_len, freq, depth, input_size, memory) in scenarios {
            ...
            assert!(
                validate_parameters(min_len, max_len, freq, depth).is_ok(),
                "Parameters failed for scenario {}",
                scenario
            );
            ...
            assert!(
                dict_size <= memory / 2,
                "Dict size exceeds memory for scenario {}",
                scenario
            );

            // Test compression ratio estimation with different data characteristics
            let text_ratio =
                estimate_compression_ratio(4.5, 0.7, dict_size as f64 / input_size as f64);
            let binary_ratio =
                estimate_compression_ratio(6.0, 0.3, dict_size as f64 / input_size as f64);
            let random_ratio =
                estimate_compression_ratio(7.8, 0.1, dict_size as f64 / input_size as f64);

            // Text should compress better than binary, binary better than random
            assert!(
                text_ratio <= binary_ratio,
                "Text ratio should be <= binary ratio for {}",
                scenario
            );
            assert!(
                binary_ratio <= random_ratio,
                "Binary ratio should be <= random ratio for {}",
                scenario
            );
