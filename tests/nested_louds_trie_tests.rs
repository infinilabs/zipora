...
use zipora::fsa::nested_louds_trie::{
    NestedLoudsTrie, NestingConfig, NestingConfigBuilder, 
    FragmentStats, NestingLevel, NestedTrieStats
};
use zipora::fsa::{
    FiniteStateAutomaton, PrefixIterable, StateInspectable, StatisticsProvider, Trie, TrieBuilder,
};
use zipora::succinct::rank_select::{
    RankSelectInterleaved256, RankSelectSeparated512, RankSelectMixedIL256,
    RankSelectSimple, RankSelectFew
};
...
fn test_basic_insert_and_lookup() {
    ...
    // Test lookups
    for key in &keys {
        assert!(trie.contains(key), "Key not found: {:?}", String::from_utf8_lossy(key));
...
fn test_hierarchical_structure_creation() {
    ...
    for key in &hierarchical_keys {
        assert!(trie.contains(key), "Missing hierarchical key: {:?}", 
...
fn test_fragment_extraction_configuration() {
    ...
    for key in &keys {
        assert_eq!(aggressive_trie.contains(key), conservative_trie.contains(key));
    }
...
fn test_backend_equivalence() {
    ...
    assert_eq!(interleaved_trie.contains(non_existent), separated_trie.contains(non_existent));
    assert_eq!(separated_trie.contains(non_existent), simple_trie.contains(non_existent));
...
fn test_backend_performance_characteristics() {
    ...
    println!("Sparse insert time: {:?}, Interleaved insert time: {:?}", 
    ...
    println!("Sparse memory: {} bytes, Interleaved memory: {} bytes", 
...
fn test_large_dataset_performance() {
    ...
    println!("Nested LOUDS Performance - Insert: {:?}, Lookup: {:?}", 
        insert_duration, lookup_duration);
    
    // Performance should be reasonable
    assert!(insert_duration.as_secs() < 60, "Insert time too slow: {:?}", insert_duration);
    assert!(lookup_duration.as_secs() < 10, "Lookup time too slow: {:?}", lookup_duration);
}
...
fn test_memory_efficiency() {
    ...
    println!("Memory efficiency - Ratio: {:.2}, Total keys: {} bytes, Trie: {} bytes", 
...
fn test_builder_interface() {
    ...
    let trie: NestedLoudsTrie<RankSelectInterleaved256> = builder.build_from_iter(keys.iter().cloned()).unwrap();
