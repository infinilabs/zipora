...
use zipora::compression::dict_zip::{
    ConcurrentSuffixArrayDictionary,
    DEFAULT_BFS_DEPTH,
...
mod compression_types_tests {
    ...
    fn test_match_creation() {
        ...
        let near_short = Match::NearShort {
            distance: 5,
            length: 3,
        };
        assert!(matches!(
        ...
        let far1_short = Match::Far1Short {
            distance: 100,
            length: 10,
        };
        assert!(matches!(
        ...
        let far2_short = Match::Far2Short {
            distance: 1000,
            length: 15,
        };
        assert!(matches!(
...
mod dictionary_tests {
    ...
    fn test_concurrent_dictionary() -> Result<()> {
        ...
        for handle in handles {
            handle
                .join()
                .map_err(|_| ZiporaError::invalid_data("Thread panicked"))??;
        }
...
mod local_matcher_tests {
    ...
    fn test_local_matcher_different_data_patterns() -> Result<()> {
        ...
        if !rep_matches.is_empty() && !rand_matches.is_empty() {
            assert!(
                rep_total_savings >= rand_total_savings || rep_matches.len() >= rand_matches.len()
            );
        }
...
mod blob_store_tests {
    ...
    fn test_blob_store_compression_roundtrip() -> Result<()> {
        ...
        builder.add_training_sample(
            b"blob store compression test with multiple patterns and data types",
        )?;
...
mod integration_tests {
    ...
    fn test_dictionary_sharing() -> Result<()> {
        ...
        assert!(
            !compressed_buffer1.is_empty(),
            "First compression should produce output"
        );
        assert!(
            !compressed_buffer2.is_empty(),
            "Second compression should produce output"
        );
...
mod performance_tests {
    ...
    fn test_compression_throughput() -> Result<()> {
        ...
        println!(
            "Compression throughput: {:.0} bytes/sec",
            compression_throughput
        );
        println!(
            "Decompression throughput: {:.0} bytes/sec",
    ...
    fn test_memory_usage_patterns() -> Result<()> {
        ...
        for (config_name, config) in configs {
            ...
            println!(
                "{} config - dict size: {} bytes, total memory: {} bytes",
