...
fn test_comparison_with_hashmap() {
    ...
    let hashmap_memory =
        hashmap.capacity() * (std::mem::size_of::<Vec<u8>>() + std::mem::size_of::<usize>());
...
fn test_simd_vs_scalar_equivalence() {
    ...
    for key in &keys {
        ...
        assert_eq!(
            trie_simd.lookup(key).is_some(),
            trie_scalar.lookup(key).is_some()
        );
    }
}
...
fn test_performance_regression_detection() {
    ...
    assert!(
        lookup_duration.as_millis() < 1000,
        "Lookup time too slow: {:?}",
        lookup_duration
    );

    println!(
        "Performance: Insert={:?}, Lookup={:?}",
