...
fn double_array_configs() -> impl Strategy<Value = DoubleArrayTrieConfig> {
    ...
    ).prop_map(|(capacity, growth, pool, simd, pool_size)| {
        DoubleArrayTrieConfig {
...
fn nesting_configs() -> impl Strategy<Value = zipora::fsa::nested_louds_trie::NestingConfig> {
    ...
    ).prop_map(|(levels, ratio, min_frag, max_frag, cache, block, density, adaptive, pool)| {
...
fn fuzz_trie_operations(data: &[u8]) -> Result<(), Box<dyn std::error::Error>> {
        ...
        match operation {
            TrieOperation::Insert(key) => {
                ...
                assert_eq!(da_result.is_ok(), nested_result.is_ok(),
                    "Insert consistency failed for key: {:?}", key);
            }
            ...
            TrieOperation::Lookup(key) => {
                ...
                assert_eq!(da_lookup.is_some(), da_trie.contains(&key),
                    "DA trie lookup/contains inconsistency for key: {:?}", key);
                assert_eq!(nested_lookup.is_some(), nested_trie.contains(&key),
                    "Nested trie lookup/contains inconsistency for key: {:?}", key);
            }
            ...
            TrieOperation::IterPrefix(prefix) => {
                ...
                for result in &da_results {
                    assert!(result.starts_with(&prefix),
                        "DA trie prefix result doesn't start with prefix: {:?} vs {:?}", result, prefix);
                }
                
                for result in &nested_results {
                    assert!(result.starts_with(&prefix),
                        "Nested trie prefix result doesn't start with prefix: {:?} vs {:?}", result, prefix);
                }
            }
            ...
            TrieOperation::IterAll => {
                ...
                assert!(da_all.len() <= reference_keys.len(),
                    "DA trie iter_all returned too many results");
                assert!(nested_all.len() <= reference_keys.len(),
            ...
            TrieOperation::Stats => {
                ...
                // Basic sanity checks
                assert!(da_stats.memory_usage < 1_000_000_000,
                    "DA trie memory usage seems too high: {}", da_stats.memory_usage);
                assert!(nested_stats.memory_usage < 1_000_000_000,
                    "Nested trie memory usage seems too high: {}", nested_stats.memory_usage);
    ...
    for key in &reference_keys {
        assert!(da_trie.contains(key), "Reference key missing from DA trie: {:?}", key);
        assert!(nested_trie.contains(key), "Reference key missing from nested trie: {:?}", key);
    }
...
fn stress_test_large_random_dataset() {
    ...
    for i in 0..10000 {
        ...
        assert!(da_result.is_ok(), "DA trie insert failed at iteration {}", i);
        assert!(nested_result.is_ok(), "Nested trie insert failed at iteration {}", i);
        ...
        if i % 1000 == 0 {
            ...
            for (ref_key, _) in reference_map.iter().take(100) {
                assert!(da_trie.contains(ref_key), 
                    "DA trie missing reference key at iteration {}: {:?}", i, ref_key);
                assert!(nested_trie.contains(ref_key),
                    "Nested trie missing reference key at iteration {}: {:?}", i, ref_key);
            }
            ...
            assert!(da_stats.memory_usage < 100_000_000, 
                "DA trie memory usage too high at iteration {}: {}", i, da_stats.memory_usage);
            assert!(nested_stats.memory_usage < 100_000_000,
                "Nested trie memory usage too high at iteration {}: {}", i, nested_stats.memory_usage);
            
            assert!(da_stats.bits_per_key < 100_000.0,
                "DA trie bits per key too high at iteration {}: {}", i, da_stats.bits_per_key);
            assert!(nested_stats.bits_per_key < 100_000.0,
                "Nested trie bits per key too high at iteration {}: {}", i, nested_stats.bits_per_key);
        }
    ...
    for (ref_key, _) in &reference_map {
        assert!(da_trie.contains(ref_key), 
            "DA trie missing reference key in final check: {:?}", ref_key);
        assert!(nested_trie.contains(ref_key),
            "Nested trie missing reference key in final check: {:?}", ref_key);
    }
    
    println!("Stress test completed successfully with {} keys", reference_map.len());
}