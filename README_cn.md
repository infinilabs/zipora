...
- **高级 Trie**：LOUDS、Critical-Bit（BMI2）、Patricia Trie + rank/select、NestTrieDawg
...
使用 `ZiporaTrie` 实现快速的词项到 ID 的映射。基于 LOUDS 的表示方式比 `HashMap<String, u32>` 节省 3-5 倍内存。
...
读密集型场景下，`DoubleArrayTrie` 提供每字节 O(1) 的状态转移。需要前缀压缩的词项存储，可使用 `NestLoudsTrieBlobStore`。
...
|-------------|-------------|----------|
| 词典 | `ZiporaTrie` | 默认选择，内存高效 |
| 词典（速度优先） | `DoubleArrayTrie` | O(1) 转移，读密集 |
