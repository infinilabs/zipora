...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!("Bit vector created with {} set bits", bit_vector.count_ones());
    println!();
    ...
    for &pos in &positions {
        ...
        println!("rank1({:>7}) = {:>6} (computed in {:>8} ns)", 
                pos, rank, time.as_nanos());
    }
    println!();
    ...
    for &k in &select_indices {
        if k < ones_count {
            ...
            println!("select1({:>7}) = {:>7} (computed in {:>8} ns)", 
                    k, pos, time.as_nanos());
        }
    }
    println!();
    ...
    for &pos in &test_positions {
        ...
        println!("Position {}: optimized={}, legacy={} {}",
                pos, optimized_rank, legacy_rank, 
                if correct { "✓" } else { "✗" });
    }
    ...
    for &k in &test_k_values {
        if k < ones_count {
            ...
            println!("Select k={}: optimized={}, legacy={} {}",
                    k, optimized_pos, legacy_pos,
                    if correct { "✓" } else { "✗" });
        }
    }
    ...
    println!("Computed {} rank operations in {:?}", batch_size, batch_time);
    println!("Average time per rank: {:?}", batch_time / batch_size as u32);
