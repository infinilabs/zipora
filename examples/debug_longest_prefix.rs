...
fn main() {
    ...
    for (i, &symbol) in test_input.iter().enumerate() {
        println!("Step {}: Processing symbol '{}' ({})", i, symbol as char, symbol);
        println!("  Current state: {}, is_final: {}", state, trie.is_final(state));
    ...
    println!("After loop: state = {}, is_final = {}", state, trie.is_final(state));
