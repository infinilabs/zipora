...
use infini_zip::{
    ...
    DictionaryBuilder, DictionaryCompressor,
    EntropyStats, HuffmanBlobStore, MemoryBlobStore,
    BlobStore
};
        ...
        Ok(tree) => {
            ...
            for &byte in b"hello " {
                if let Some(code) = tree.get_code(byte) {
                    let code_str: String = code.iter().map(|&b| if b { '1' } else { '0' }).collect();
            ...
            match huffman_store.put(test_data) {
                Ok(id) => {
                    ...
                    if stats.compressions > 0 {
                        println!("   Average compression time: {:.1} μs", 
