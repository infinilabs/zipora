...
fn main() {
    // Enable debug output
    unsafe { env::set_var("SORTABLE_DEBUG", "1"); }
    ...
    // Test 2: Long strings (might use radix if threshold is low)
    {
        ...
        let mut vec = SortableStrVec::new();
        for i in 0..1000 {
            let s = format!("this_is_a_very_long_string_for_testing_radix_sort_algorithm_selection_{:030}", i);
    ...
    // Test 3: Performance comparison - SortableStrVec vs Vec<String>
    {
        ...
        // Test SortableStrVec with new algorithm selection
        {
            ...
            println!("  Memory: {} bytes (vs Vec<String>: {} bytes)", our_size, vec_size);
            println!("  Savings: {:.1}%\n", savings * 100.0);
        }
    ...
    // Test 4: Force radix sort with environment variable
    {
        ...
        unsafe { env::set_var("SORTABLE_STRVEC_MIN_RADIX_LEN", "5"); }  // Very low threshold
        ...
        // Reset env var
        unsafe { env::remove_var("SORTABLE_STRVEC_MIN_RADIX_LEN"); }
    }
