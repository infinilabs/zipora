...
fn main() {
    ...
    for &size in &sizes {
        ...
        println!("  Push: ValVec32 {:?}, std::Vec {:?} (ratio: {:.2}x)",
                 valvec_push_time, stdvec_push_time,
                 valvec_push_time.as_secs_f64() / stdvec_push_time.as_secs_f64());
        println!("  Iter: ValVec32 {:?}, std::Vec {:?} (ratio: {:.2}x)",
                 valvec_iter_time, stdvec_iter_time,
    ...
    for i in 0..20 {
        ...
        if new_capacity != old_capacity {
            ...
            if old_capacity > 0 {
                ...
                println!("Growth at element {}: {} -> {} (ratio: {:.3}x, target: 1.609x)",
