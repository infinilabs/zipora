...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    
    {
        ...
        output.truncate()?;
        println!("✅ Truncated file to actual data size: {} bytes", output.capacity());
    ...
    
    {
        ...
        println!("   Read u32: 0x{:08X} ({})", value1, if value1 == 0x12345678 { "✅ correct" } else { "❌ incorrect" });
        
        let value2 = input.read_u64()?;
        println!("   Read u64: 0x{:016X} ({})", value2, if value2 == 0x9ABCDEF012345678 { "✅ correct" } else { "❌ incorrect" });
        
        let var_int = input.read_var_int()?;
        println!("   Read var_int: {} ({})", var_int, if var_int == 300 { "✅ correct" } else { "❌ incorrect" });
        
        let text = input.read_length_prefixed_string()?;
        println!("   Read string: \"{}\" ({})", text, if text == "Hello, Memory Mapping!" { "✅ correct" } else { "❌ incorrect" });
    ...
    
    {
        ...
        println!("   Position after peek: {} (should still be 0)", input.position());
