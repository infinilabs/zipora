...
fn demo_basic_allocation() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "✓ Created secure memory pool with {} byte chunks",
    ...
    println!(
        "✓ Pool stats: {} allocs, {} deallocs, {} corruptions detected",
...
fn demo_security_features() -> Result<(), Box<dyn std::error::Error>> {
    ...
    let stats = pool.stats();
    println!(
        "✓ Security stats: {} double-free attempts blocked, {} corruptions detected",
...
fn demo_thread_safety() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "✓ Completed {} concurrent allocations in {:?}",
        total, duration
    );
    println!(
...
fn demo_performance_comparison() -> Result<(), Box<dyn std::error::Error>> {
    ...
    if ratio < 1.0 {
        println!(
            "  🚀 Secure pool is {:.1}x FASTER than standard allocator!",
            1.0 / ratio
        );
    } else {
        println!(
            "  📊 Secure pool is {:.1}x slower than standard allocator",
            ratio
        );
...
fn demo_global_pools() -> Result<(), Box<dyn std::error::Error>> {
    ...
    for size in sizes {
        ...
        println!(
            "Size {} → class {} → pool with {} byte chunks",
            size,
            size_class,
            pool.config().chunk_size
        );
        ...
        let ptr = pool.allocate()?;
        println!(
            "  ✓ Allocated from global pool, generation {}",
            ptr.generation()
        );
        drop(ptr);
    }
...
fn demo_statistics_monitoring() -> Result<(), Box<dyn std::error::Error>> {
    ...
    println!(
        "│ Cross-thread steals     │ {:>11} │",
        stats.cross_thread_steals
    );
    println!(
        "│ Corruptions detected    │ {:>11} │",
        stats.corruption_detected
    );
    println!(
        "│ Double-free attempts    │ {:>11} │",
