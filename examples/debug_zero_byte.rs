...
fn compare_keys_bitwise(key1: &[u8], key2: &[u8]) {
    ...
    println!("Key1: {:?} (len={})", std::str::from_utf8(key1).unwrap_or("<invalid>"), key1.len());
    println!("Key2: {:?} (len={})", std::str::from_utf8(key2).unwrap_or("<invalid>"), key2.len());
    ...
    for i in 0..max_len {
        ...
        if i < min_len {
            println!("  Byte {}: {:02x} vs {:02x} -> {:08b} vs {:08b}", i, byte1, byte2, byte1, byte2);
            if byte1 != byte2 {
            ...
            println!("  Byte {}: {} vs {} (length difference)", i,
                     if i < key1.len() { format!("{:02x}", byte1) } else { "N/A".to_string() },
                     if i < key2.len() { format!("{:02x}", byte2) } else { "N/A".to_string() });
        }
    }
...
fn main() -> Result<()> {
    ...
    println!("Inserting key2: {:?} (with zero byte)", std::str::from_utf8(&key2[..key2.len()-1]).unwrap());
    trie.insert(key2)?;
    println!("  -> Trie now has {} keys", trie.len());
    ...
    println!("Looking up key1 {:?}: {}", std::str::from_utf8(key1).unwrap(), found1);
    
    let found2 = trie.contains(key2);
    println!("Looking up key2 {:?} (with zero): {}", std::str::from_utf8(&key2[..key2.len()-1]).unwrap(), found2);
