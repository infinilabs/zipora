...
fn benchmark_compression_ratio(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        let test_cases = vec![
            ...
            (
                "nearly_identical",
                TestDataGenerator::nearly_identical(size),
            ),
        ...
        for (pattern, data) in test_cases {
            ...
            group.bench_with_input(
                ...
                |b, data| {
                    b.iter(|| {
                        ...
                        if pattern == "sorted"
                            || pattern == "small_range"
                            || pattern == "nearly_identical"
                        {
                            assert!(
                                ratio < 0.5,
                                "Should achieve >50% compression for {}",
                                pattern
                            );
                        }
            ...
            group.bench_with_input(
                BenchmarkId::new(
                    "UintVector",
                    format!("{}_{}_{}", pattern, size, "compression"),
                ),
                &data,
                |b, data| {
                    b.iter(|| {
...
fn benchmark_construction(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        group.bench_with_input(BenchmarkId::new("IntVec", size), &data, |b, data| {
            b.iter(|| IntVec::<u32>::from_slice(black_box(data)).unwrap())
        });

        group.bench_with_input(BenchmarkId::new("UintVector", size), &data, |b, data| {
            b.iter(|| UintVector::build_from(black_box(data)).unwrap())
        });
...
fn benchmark_integer_types(c: &mut Criterion) {
    ...
    let i8_data: Vec<i8> = (0..size)
        .map(|i| ((i % 256) as i8).wrapping_sub(127))
        .collect();
    let i16_data: Vec<i16> = (0..size)
        .map(|i| ((i % 65536) as i16).wrapping_sub(32767))
        .collect();
...
fn benchmark_large_datasets(c: &mut Criterion) {
    ...
    for size in sizes {
        ...
        group.bench_with_input(BenchmarkId::new("IntVec_large", size), &data, |b, data| {
            b.iter(|| {
                let compressed = IntVec::<u32>::from_slice(black_box(data)).unwrap();
                ...
                assert!(
                    ratio < 0.5,
                    "Should achieve good compression even for large datasets"
                );
