    ...
    for &size in SMALL_SIZES {
        for pattern in &test_patterns {
            ...
            let configs = vec![
                ...
                (
                    "high_performance",
                    SeparatedStorageConfig::high_performance().build(),
                ),
                ("low_memory", SeparatedStorageConfig::low_memory().build()),
                (
                    "adaptive",
                    SeparatedStorageConfig::analyze_and_optimize(&bv).build(),
                ),
            ];
...
fn bench_memory_overhead_analysis(c: &mut Criterion) {
    ...
    let block_configs = vec![
        (
            "block_256",
            SeparatedStorageConfig::new().block_size(256).build(),
        ),
        (
            "block_512",
            SeparatedStorageConfig::new().block_size(512).build(),
        ),
        (
            "block_1024",
            SeparatedStorageConfig::new().block_size(1024).build(),
        ),
    ];
    ...
    for (config_name, config) in block_configs {
        ...
        println!(
            "Config {}: estimated overhead = {:.2}%",
            config_name,
            config.estimated_memory_overhead()
        );

        let rs = RankSelectSeparated256::with_config(bv.clone(), config).unwrap();
        let actual_overhead = rs.space_overhead_percent();
        println!(
            "Config {}: actual overhead = {:.2}%",
            config_name, actual_overhead
        );
    ...
    for (config_name, config) in select_configs {
        ...
        if ones_count > 100 {
            let select_indices: Vec<usize> =
                (0..ones_count).step_by((ones_count / 100).max(1)).collect();
...
fn bench_adaptive_selection(c: &mut Criterion) {
    ...
    for (size, pattern) in test_cases {
        ...
        let configs = vec![("manual", manual_config), ("adaptive", adaptive_config)];
...
fn bench_scalability(c: &mut Criterion) {
    ...
    for &size in SIZES {
        ...
        let num_ops = 1000;
        let test_positions: Vec<usize> = (0..num_ops).map(|i| (i * size) / num_ops).collect();
        ...
        if ones_count > num_ops {
            let select_indices: Vec<usize> =
                (0..num_ops).map(|i| (i * ones_count) / num_ops).collect();
...
fn bench_access_patterns(c: &mut Criterion) {
    ...
    let bv = create_bit_vector(size, DataPattern::Uniform(0.5));
    let rs =
        RankSelectSeparated256::with_config(bv, SeparatedStorageConfig::high_performance().build())
            .unwrap();
    ...
    let access_patterns = vec![
        (
            "sequential",
            (0..size).step_by(size / 1000).collect::<Vec<_>>(),
        ),
        ("random", {
