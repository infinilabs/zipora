...
fn generate_prefix_heavy_keys(count: usize) -> Vec<Vec<u8>> {
    ...
    let prefixes = vec!["app", "application", "compress", "data", "engine", "fast", "graph"];
...
fn bench_double_array_trie_prefix_iteration(c: &mut Criterion) {
    ...
    let prefixes = vec![b"app".as_slice(), b"compress".as_slice(), b"data".as_slice()];
