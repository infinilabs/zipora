...
use zipora::memory::{
    MemoryPool, PoolConfig, SecureMemoryPool, SecurePoolConfig, get_global_pool_for_size,
...
fn bench_size_classes(c: &mut Criterion) {
    ...
    group.bench_function("large_1mb", |b| {
        ...
        b.iter(|| {
            for _ in 0..1000 {
                // Fewer iterations for large allocations
                let ptr = pool.allocate().unwrap();
                black_box(&ptr);
                drop(ptr);
            }
...
fn bench_statistics_collection(c: &mut Criterion) {
    ...
    group.bench_function("global_stats", |b| {
        ...
        let _ptrs: Vec<_> = (0..100)
            .map(|_| get_global_pool_for_size(1024).allocate().unwrap())
            .collect();
