...
use zipora::{
    CacheAlignedVec, FastVec, get_numa_stats, set_current_numa_node, 
    init_numa_pools, clear_numa_pools, get_optimal_numa_node,
};
...
fn bench_vector_operations(c: &mut Criterion) {
    ...
    for size in [1000, 10000, 100000].iter() {
        ...
        // CacheAlignedVec
        group.bench_with_input(BenchmarkId::new("cache_aligned_vec_push", size), size, |b, &size| {
            b.iter(|| {
        ...
        group.bench_with_input(BenchmarkId::new("cache_aligned_vec_with_capacity", size), size, |b, &size| {
            b.iter(|| {
...
fn bench_sequential_access(c: &mut Criterion) {
    ...
    for size in [10000, 100000, 1000000].iter() {
        ...
        // Standard Vec
        group.bench_with_input(BenchmarkId::new("std_vec_sequential", size), size, |b, &size| {
            let vec: Vec<i32> = (0..size as i32).collect();
            b.iter(|| {
                let mut sum = 0i32;
                for val in &vec {
        ...
        // CacheAlignedVec
        group.bench_with_input(BenchmarkId::new("cache_aligned_vec_sequential", size), size, |b, &size| {
            let mut vec = CacheAlignedVec::with_capacity(size).unwrap();
            for i in 0..size as i32 {
                vec.push(i).unwrap();
            }
        ...
        group.bench_with_input(BenchmarkId::new("cache_aligned_vec_slice", size), size, |b, &size| {
            let mut vec = CacheAlignedVec::with_capacity(size).unwrap();
            for i in 0..size as i32 {
                vec.push(i).unwrap();
            }
