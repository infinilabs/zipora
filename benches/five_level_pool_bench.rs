...
use std::thread;
use zipora::memory::{
    AdaptiveFiveLevelPool, ConcurrencyLevel, FiveLevelPoolConfig, FixedCapacityPool, LockFreePool,
    MutexBasedPool, NoLockingPool, ThreadLocalPool,
};
...
fn bench_no_locking_pool(c: &mut Criterion) {
    ...
    for config in [
        ...
        group.bench_with_input(
            BenchmarkId::new(
                "alloc_free",
                format!("{}x{}", config.iterations, config.allocation_size),
            ),
            &config,
            |b, config| {
                b.iter(|| {
...
fn bench_mutex_based_pool(c: &mut Criterion) {
    ...
    for config in [BenchConfig::small(), BenchConfig::medium()] {
        group.throughput(Throughput::Elements(
            (config.iterations * config.thread_count) as u64,
        ));

        group.bench_with_input(
            BenchmarkId::new(
                "concurrent_alloc_free",
                format!(
                    "{}threads_{}x{}",
                    config.thread_count, config.iterations, config.allocation_size
                ),
            ),
            &config,
            |b, config| {
                        ...
                        let handles: Vec<_> = (0..config.thread_count)
                            .map(|_| {
                                ...
                                thread::spawn(move || {
                                    ...
                                    for _ in 0..iterations {
                                        let offset =
                                            handle.alloc(black_box(allocation_size)).unwrap();
                                        offsets.push(offset);
                                    }
...
fn bench_level_comparison(c: &mut Criterion) {
    ...
    group.bench_function("level1_no_locking", |b| {
        b.iter(|| {
            let pool_config = FiveLevelPoolConfig::performance_optimized();
            let mut pool =
                AdaptiveFiveLevelPool::with_level(pool_config, ConcurrencyLevel::SingleThread)
                    .unwrap();
...
fn bench_fragmentation_patterns(c: &mut Criterion) {
    ...
    let patterns = [("sequential", false), ("interleaved", true)];
