...
fn bench_construction_double_array(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000].iter() {
        ...
        group.bench_with_input(format!("incremental_{}", size), size, |b, &_size| {
            b.iter_batched(
                || keys.clone(),
                |keys| {
        ...
        group.bench_with_input(format!("builder_sorted_{}", size), size, |b, &_size| {
            b.iter_batched(
                || keys.clone(),
                |keys| {
                    black_box(
        ...
        group.bench_with_input(format!("builder_unsorted_{}", size), size, |b, &_size| {
            b.iter_batched(
                || {
...
fn bench_lookup_performance(c: &mut Criterion) {
    ...
    for size in [100, 1000, 10000].iter() {
        ...
        group.bench_with_input(format!("double_array_{}", size), &keys, |b, keys| {
            b.iter(|| {
                for key in keys {
        ...
        // LOUDS Trie lookup
        group.bench_with_input(format!("louds_{}", size), &keys, |b, keys| {
            b.iter(|| {
                for key in keys {
        ...
        // CritBit Trie lookup
        group.bench_with_input(format!("critbit_{}", size), &keys, |b, keys| {
            b.iter(|| {
                for key in keys {
        ...
        // HashMap lookup
        group.bench_with_input(format!("hashmap_{}", size), &keys, |b, keys| {
            b.iter(|| {
                for key in keys {
        ...
        // BTreeMap lookup
        group.bench_with_input(format!("btreemap_{}", size), &keys, |b, keys| {
            b.iter(|| {
                for key in keys {
...
fn bench_memory_efficiency(c: &mut Criterion) {
    ...
    for &size in &sizes {
        ...
        group.bench_with_input(format!("memory_usage_{}", size), &size, |b, &_size| {
            b.iter_batched(
                || keys.clone(),
                |keys| {
                    let mut trie = DoubleArrayTrie::new();
...
fn bench_scaling_performance(c: &mut Criterion) {
    ...
    for &size in &[1000, 5000, 10000, 25000] {
        ...
        group.bench_with_input(format!("lookup_scaling_{}", size), &keys, |b, keys| {
            let mut trie = DoubleArrayTrie::new();
            for key in keys {
                trie.insert(key).unwrap();
            }
...
fn bench_config_variants(c: &mut Criterion) {
    ...
    for (name, config) in &configs {
        group.bench_with_input(format!("construction_{}", name), config, |b, config| {
            b.iter_batched(
                || keys.clone(),
                |keys| {
...
criterion_group!(workload_benches, bench_realistic_workloads);
