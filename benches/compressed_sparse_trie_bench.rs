...
use zipora::fsa::{
    CompressedSparseTrie, ConcurrencyLevel,
    PatriciaTrie,
    LoudsTrie,
    Trie,
};
...
fn generate_sparse_keys(count: usize, sparsity: f64) -> Vec<Vec<u8>> {
    ...
    let mut unique_keys: Vec<Vec<u8>> = keys.into_iter().collect::<HashSet<_>>().into_iter().collect();
    unique_keys.sort();
    unique_keys
}
...
fn bench_csp_lookup(c: &mut Criterion) {
    ...
    for &level in &levels {
        ...
        group.bench_function("sequential", |b| {
            b.to_async(&rt).iter_batched(
                || {
                    let mut trie = if level == ConcurrencyLevel::NoWriteReadOnly {
                        // For read-only, we need to populate first with a different level
                        let mut temp_trie = CompressedSparseTrie::new(ConcurrencyLevel::SingleThreadStrict).unwrap();
                        for key in &keys {
                            temp_trie.insert(key).unwrap();
                        }
                        // Convert to read-only (conceptually - in real implementation you'd have a different constructor)
        ...
        if level != ConcurrencyLevel::NoWriteReadOnly {
            group.bench_function("with_tokens", |b| {
                b.to_async(&rt).iter_batched(
                    ...
                    |(trie, keys)| async move {
                        let token = trie.acquire_reader_token().await.unwrap();
                        for key in keys {
                            black_box(trie.contains_with_token(&key, &token));
                        }
                    },
                    BatchSize::SmallInput,
                );
                    ...
                    |(trie, keys)| async move {
                        let handles: Vec<_> = (0..thread_count)
                            .map(|i| {
                                ...
                                let thread_keys = keys[i * keys_per_thread..(i + 1) * keys_per_thread].to_vec();
                                ...
                                tokio::spawn(async move {
                                    ...
                                    for key in thread_keys {
                                        black_box(trie_guard.insert_with_token(&key, &writer_token).unwrap());
                                    }
                                })
                            })
                            .collect();
    ...
    for (dataset_name, keys) in datasets {
        ...
        group.bench_function("louds_trie", |b| {
            b.iter_batched(
                ...
                |keys| {
                    black_box(LoudsTrie::build_from_sorted(keys).unwrap())
                },
                BatchSize::SmallInput,
            );
        });
...
fn bench_memory_efficiency(c: &mut Criterion) {
    ...
    for &key_count in &key_counts {
        for &sparsity in &sparsity_levels {
            ...
            c.bench_with_input(
                BenchmarkId::new("memory_efficiency", format!("{}keys_{}sparse", key_count, (sparsity * 100.0) as u32)),
                &keys,
                |b, keys| {
...
fn bench_lock_free_operations(c: &mut Criterion) {
    ...
    let contention_levels = [1, 2, 4, 8, 16];
            ...
            |b, keys| {
                b.to_async(&rt).iter_batched(
                    || {
                        ...
                        (Arc::new(tokio::sync::RwLock::new(trie)), keys.clone())
                    },
                    |(trie, keys)| async move {
                        ...
                        let handles: Vec<_> = (0..readers)
                            .map(|i| {
                                let trie_clone = trie.clone();
                                let reader_keys = keys[i * keys_per_reader..(i + 1) * keys_per_reader].to_vec();
                                
                                tokio::spawn(async move {
                                    let trie_guard = trie_clone.read().await;
                                    let reader_token = trie_guard.acquire_reader_token().await.unwrap();
                                    
                                    for key in reader_keys {
                                        black_box(trie_guard.contains_with_token(&key, &reader_token));
                                    }
                                })
                            })
                            .collect();
...
fn bench_key_patterns(c: &mut Criterion) {
    ...
    let patterns = [
        ("short_keys", (0..key_count).map(|i| format!("k{}", i).into_bytes()).collect::<Vec<_>>()),
        ("long_keys", (0..key_count).map(|i| format!("very_long_key_with_lots_of_characters_{:08x}", i).into_bytes()).collect::<Vec<_>>()),
        ("common_prefix", (0..key_count).map(|i| format!("common_prefix_for_compression_test_{:06x}", i).into_bytes()).collect::<Vec<_>>()),
        ("random_length", (0..key_count).map(|i| {
            let len = 5 + (i % 20);
            format!("{:0width$x}", i, width = len).into_bytes()
        }).collect::<Vec<_>>()),
    ];
