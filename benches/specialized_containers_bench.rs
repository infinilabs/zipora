...
fn bench_circular_queue_operations(c: &mut Criterion) {
    ...
    for &size in SIZES {
        ...
        group.bench_with_input(
            BenchmarkId::new("AutoGrowCircularQueue", size),
            &size,
            |b, &size| {
                b.iter(|| {
...
fn bench_fixed_str_vec_operations(c: &mut Criterion) {
    ...
    for &size in SIZES {
        ...
        let test_strings: Vec<String> = (0..size).map(|i| format!("test{:011}", i)).collect();

        group.bench_with_input(
            BenchmarkId::new("FixedStr16Vec", size),
            &size,
            |b, &_size| {
                b.iter(|| {
...
fn bench_sortable_str_vec_sorting(c: &mut Criterion) {
    ...
    for &size in &sizes {
        ...
        group.bench_with_input(
            BenchmarkId::new("SortableStrVec", size),
            &size,
            |b, &_size| {
                b.iter(|| {
